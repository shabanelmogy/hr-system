...
describe("parseLoginResult", () => {
  ...
  it("accepts a tenant-selection challenge for one tenant", () => {
    const response = {
      isAuthenticated: false,
      requiresTenantSelection: true,
      tenantSelectionToken: "tenant-token",
      tenantSelectionTokenExpiration: "2026-08-14T12:00:00Z",
      tenants: [{ id: "tenant-a", identifier: "A", name: "Tenant A" }],
    };
  ...
  it("accepts a company-selection challenge for one company", () => {
    const response = {
      isAuthenticated: false,
      requiresCompanySelection: true,
      companySelectionToken: "temporary-token",
