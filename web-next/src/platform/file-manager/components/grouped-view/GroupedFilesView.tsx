...
const GroupedFilesView: React.FC<GroupedFilesViewProps> = ({
  ...
  onOpenGroup,
  onGroupChange,
  onBackToGroups: onBackToGroupsRef,
}) => {
  ...
  useEffect(() => {
    if (onBackToGroupsRef) {
      onBackToGroupsRef.current = handleBackToGroups;
    }
    return () => {
      if (onBackToGroupsRef?.current === handleBackToGroups) {
        onBackToGroupsRef.current = null;
      }
    };
  }, [handleBackToGroups, onBackToGroupsRef]);
