...
export default function JobDescriptionDetailsDialog({
  ...
  type StatusKey = "draft" | "approved" | "rejected" | "expired";
  const statusKey: StatusKey = typeof item.jobDescriptionStatus === "number"
    ? (({ 1: "draft", 2: "approved", 3: "rejected", 4: "expired" } as const)[item.jobDescriptionStatus] ?? "draft")
    : (item.jobDescriptionStatus?.toLowerCase() as StatusKey | undefined) ?? "draft";
