  ...
  return <Box sx={{ minHeight: 0, display: "flex", flexDirection: "column", gap: 1 }}>
    <Paper variant="outlined" sx={{ minHeight: 430, overflow: "auto", position: "relative" }}>
      <MyDataGrid rows={rows} columns={columns} loading={loading} checkboxSelection={false} autoSelectFirstRow={false}
        ...
        toolbarSearch={searchValue !== undefined && searchPlaceholder && onSearchChange ? { value: searchValue, placeholder: searchPlaceholder, onChange: onSearchChange, onClear: () => onSearchChange("") } : undefined}
    ...
    {error ? <Alert severity="error" action={<Button onClick={onRetry}>{t("common.retry")}</Button>}>{extractErrorMessage(error)}</Alert> : null}
  </Box>;
}
