...
export default function AttendanceDevicesPage() {
  ...
  const deviceItems = useMemo(() => devices.data?.items ?? [], [devices.data?.items]);

  const selected = useMemo(
    () => (selectedId === null ? null : deviceItems.find((item) => item.id === selectedId) ?? null),
    [deviceItems, selectedId],
  );
  ...
  const agentOnline = selectedAgent?.lastSeenAtUtc
    ? now - new Date(selectedAgent.lastSeenAtUtc).getTime() < 90_000
    : false;
