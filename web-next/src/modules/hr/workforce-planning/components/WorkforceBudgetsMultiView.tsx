...
export default function WorkforceBudgetsMultiView(props: Props) {
  ...
  return <Box sx={{ display: "flex", flexDirection: "column", height: "100%", width: "100%", minWidth: 0 }}>
    <PageHeader variant="multi-view" title={t("workforceBudget.title")} storageKey="workforce-budgets-view-layout" defaultView="grid" availableViews={["grid", "cards"]} onAdd={props.permissions.canManage ? props.onAdd : undefined} dataCount={props.totalCount} totalLabel={t("workforceBudget.totalLabel")} onRefresh={props.onRefresh} onViewTypeChange={value => (value === "grid" || value === "cards") && setView(value)} onFilter={() => setFiltersVisible(value => !value)} isFilterBarVisible={filtersVisible} showActions={{ add: props.permissions.canManage, refresh: true, export: false, filter: true }} />
