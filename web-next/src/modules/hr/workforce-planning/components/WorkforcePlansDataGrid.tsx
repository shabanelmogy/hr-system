...
export default function WorkforcePlansDataGrid(props: Props) {
  ...
  const columns = useMemo<GridColDef<WorkforcePlanListItem>[]>(() => [
    ...
    { field: "status", headerName: t("workforcePlanning.fields.status"), minWidth: 170, renderCell: ({ row }) => <Stack direction="row" spacing={.5} sx={{ height: "100%", alignItems: "center" }}><Chip size="small" color={row.status === 4 ? "success" : row.status === 5 ? "error" : row.status === 3 ? "warning" : "info"} label={t(`workforcePlanning.status.${statusKeys[row.status]}`)} />{row.isDeleted ? <Chip size="small" variant="outlined" label={t("workforcePlanning.recordStatus.archived")} /> : null}</Stack> },
