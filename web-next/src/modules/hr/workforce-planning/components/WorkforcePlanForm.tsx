...
export default function WorkforcePlanForm({ open, mode, item, revisions = [], loading = false, detailError, onRetryDetail, onClose, onSubmit }: Props) {
  ...
  const periodOptions = useMemo<PeriodOption[]>(() => (fiscalYear.data?.periods ?? []).map(value => ({ id: value.id, sequence: value.sequence, displayName: `${value.code} Ã¢â‚¬â€ ${isArabic ? value.nameAr : value.nameEn}` })), [fiscalYear.data?.periods, isArabic]);
  ...
  const generateMockData = () => {
    ...
    form.setValue("titleAr", "Ã˜Â®Ã˜Â·Ã˜Â© Ã™â€ Ã™â€¦Ã™Ë† Ã˜Â§Ã™â€žÃ™â€šÃ™Ë†Ã™â€° Ã˜Â§Ã™â€žÃ˜Â¹Ã˜Â§Ã™â€¦Ã™â€žÃ˜Â© Ã˜Â§Ã™â€žÃ˜Â³Ã™â€ Ã™Ë†Ã™Å Ã˜Â©", next);
