...
export default function InterviewEvaluationDialog({
  ...
  const scorecardKey = `${interviewId ?? "none"}:${template?.skills?.map((skill) => `${skill.skillName}:${skill.defaultWeightPercentage ?? ""}`).join("|") ?? ""}`;
  ...
  const [ratingsState, setRatingsState] = useState<{ key: string; value: SkillRatingState[] }>({ key: "", value: [] });
  const ratings = ratingsState.key === scorecardKey ? ratingsState.value : defaultRatings;
  const [recommendationState, setRecommendationState] = useState<{
    key: string;
    value: InterviewRecommendation;
  }>({ key: "", value: InterviewRecommendation.Hire });
  const recommendation = recommendationState.key === scorecardKey
    ? recommendationState.value
    : InterviewRecommendation.Hire;
  const [commentsState, setCommentsState] = useState({ key: "", value: "" });
  const comments = commentsState.key === scorecardKey ? commentsState.value : "";

  const handleScoreChange = (index: number, newScore: number) => {
    setRatingsState((previous) => ({
      key: scorecardKey,
      value: (previous.key === scorecardKey ? previous.value : ratings).map((item, i) => (i === index ? { ...item, score: newScore } : item)),
    }));
  };
  ...
  const handleNotesChange = (index: number, newNotes: string) => {
    setRatingsState((previous) => ({
      key: scorecardKey,
      value: (previous.key === scorecardKey ? previous.value : ratings).map((item, i) => (i === index ? { ...item, notes: newNotes } : item)),
    }));
  };
      ...
      <Stack spacing={2.5} sx={{ mt: 1 }}>
        ...
        <FormControl fullWidth size="small">
          ...
          <Select
            ...
            onChange={(e) => setRecommendationState({
              key: scorecardKey,
              value: Number(e.target.value) as InterviewRecommendation,
            })}
          >
        ...
        <TextField
          ...
          value={comments}
          onChange={(e) => setCommentsState({ key: scorecardKey, value: e.target.value })}
          multiline
          rows={3}
          fullWidth
