...
describe("FiscalYearService", () => {
  ...
  it("accepts API-valid single-character names and rejects control characters", () => {
    const schema = getFiscalYearSchema(t);
    expect(schema.safeParse({ ...request, code: "FY-2027", nameAr: "س", nameEn: "A" }).success).toBe(true);
    expect(schema.safeParse({ ...request, code: "FY-2027", nameAr: "س", nameEn: "A\u0007" }).success).toBe(false);
  });
