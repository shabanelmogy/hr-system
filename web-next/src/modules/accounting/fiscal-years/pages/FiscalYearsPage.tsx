...
export default function FiscalYearsPage() {
  ...
  const [lifecycleAction, setLifecycleAction] = useState<FiscalYearLifecycleAction | null>(null);
  const query = useMemo(() => ({
  ...
  const currentItem: FiscalYearDetail | null = dialog === "add" ? null : (details.data ?? null);
  ...
  const fail = async (error: Error, key: string) => {
    if (error instanceof ApiClientError && error.status === 409 && error.code === "ConcurrencyConflict") {
      ...
      showToast.warning(t("fiscalYears.messages.conflictReloaded"));
      return;
    }
    showToast.error(extractErrorMessage(error) || t(key));
  };
  const create = useCreateFiscalYear({ onSuccess: item => { showToast.success(t("fiscalYears.messages.created", { code: item.code })); setDialog(null); }, onError: error => { void fail(error, "fiscalYears.messages.createError"); } });
  const update = useUpdateFiscalYear({ onSuccess: item => { showToast.success(t("fiscalYears.messages.updated", { code: item.code })); setDialog(null); }, onError: error => { void fail(error, "fiscalYears.messages.updateError"); } });
