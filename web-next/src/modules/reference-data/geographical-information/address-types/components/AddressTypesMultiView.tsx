...
export default function AddressTypesMultiView(props: AddressTypesMultiViewProps) {
  ...
  const visibleView =
    (currentView === "import" && !props.permissions.canCreate) ||
    (currentView === "report" && !canViewReports)
      ? "grid"
      : currentView;
  const handleViewChange = useCallback((view: string) => {
    if (view !== "grid" && view !== "cards" && view !== "chart" && view !== "report" && view !== "import") return;
    if ((view !== "import" || permissions.canCreate) && (view !== "report" || canViewReports)) {
      if (view === "chart" && page !== 0) onPageChange(0);
      setCurrentView(view);
    }
  }, [canViewReports, onPageChange, page, permissions.canCreate]);
  ...
  const handleGridSortChange = useCallback((model: GridSortModel) => {
    const next = model[0];
    if (next?.sort && sortableColumns.has(next.field as AddressTypeSortColumn)) onSortChange(next.field as AddressTypeSortColumn, next.sort.toUpperCase() as "ASC" | "DESC");
  }, [onSortChange]);
  ...
  const availableViews: AddressTypeView[] = [
    "grid",
    "cards",
    "chart",
    ...(canViewReports ? ["report" as const] : []),
    ...(props.permissions.canCreate ? ["import" as const] : []),
  ];
