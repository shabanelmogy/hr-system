...
const StateForm = dynamic(() => import("../components/StateForm"), { ssr: false });
const StateArchiveDialog = dynamic(() => import("../components/StateArchiveDialog"), { ssr: false });
const StateRestoreDialog = dynamic(() => import("../components/StateRestoreDialog"), { ssr: false });
const StateBulkArchiveDialog = dynamic(() => import("../components/StateBulkArchiveDialog"), { ssr: false });
...
const StatesPage = () => {
  ...
  const formState = formDialogType === "add" ? null : (details.data ?? null);
  ...
  return <>
    ...
    {logic.dialogType === "delete" ? <StateArchiveDialog open selectedState={logic.selectedState} loading={logic.isArchiving} onClose={logic.closeDialog} onConfirm={logic.handleDelete} /> : null}
    {logic.restoreState ? <StateRestoreDialog state={logic.restoreState} loading={logic.isRestoring} onClose={logic.closeRestore} onConfirm={() => void logic.handleRestore()} /> : null}
    {logic.bulkArchiveOpen ? <StateBulkArchiveDialog open selectedCount={logic.selectedStateIds.length} loading={logic.isBulkArchiving} onClose={logic.closeBulkArchive} onConfirm={logic.handleBulkArchive} /> : null}
  </>;
};
