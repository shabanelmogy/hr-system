...
describe("appointment calendar adapter", () => {
  ...
  it("trusts the API isAllDay flag for 24-hour timed appointments", () => {
    const event = appointmentToCalendarEvent({
      id: 4,
      text: "Timed maintenance",
    ...
    expect(event.allDay).toBe(false);
    expect(event.start).toBe("2026-07-20T12:00:00.000Z");
    expect(event.end).toBe("2026-07-21T12:00:00.000Z");
  });
});
