...
interface GlobalSearchBarProps {
  ...
  className?: string;
  onResultSelect?: (result: any, route: string) => void;
  [key: string]: any;
}
            ...
            <>
              {hasResults ? (
                <>
                  ...
                  {Object.entries(resultsByEntity).map(([entityType, entityResults]: [string, any]) => {
                    const entityConfig = activeEntities[entityType];
                    if (!entityConfig || (entityResults as any[]).length === 0) return null;
                    ...
                    return (
                      <div key={entityType} className="search-section">
                        <div className="search-section-header">
                          ...
                          <span className="result-count">({(entityResults as any[]).length})</span>
                        </div>
                        
                        {(entityResults as any[]).slice(0, 3).map((result, index) => {
