...
class SimpleNotificationSystem {
  ...
  test: (action: string, name: string) => void;
   ...
   */
  registerHandler(entityType: string, handler: any) {
  ...
  /**
   * Start SignalR connection (deprecated - now handled by SignalRProvider)
   * @deprecated SignalR is now started automatically by SignalRProvider
   */
  startSignalR() {
    console.warn("[SimpleNotificationSystem] startSignalR() is deprecated. SignalR is now managed by SignalRProvider.");
    // Do nothing - SignalR is started by the provider
  }

  /**
   * Add notification (called by handlers)
   */
  addNotification(message: string, type = "info", options: any = {}) {
    const notification = {
      id: this.generateId(),
      message,
      type,
