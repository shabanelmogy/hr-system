...
const ServerReportViewerClient = ({ featureKey, labels }: ServerReportViewerProps) => {
  ...
  const effectiveSelectedId = selectedId && templates.some((template) => template.id === selectedId)
    ? selectedId
    : (templates[0]?.id ?? "");

  const detailQuery = useQuery({
    queryKey: ["report-templates", "published", featureKey, effectiveSelectedId],
    queryFn: () => reportTemplateService.getById(effectiveSelectedId),
