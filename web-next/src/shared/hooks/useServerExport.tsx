...
const useServerExport = ({
  ...
  /**
   * Export data to Excel based on current grid state
   */
  const exportToExcel = async ({
    ...
    try {
      ...
      const baseUrl = apiRoutes.export.excel || "/api/v1/Export/ExportExcel";
      const apiUrl = `${baseUrl}/${fileName}/${culture}`;
      console.log(
      ...
      const response = await (apiService as any).api({
        method: "post",
        url: apiUrl,
        data: formattedData,
        responseType: "blob",
      ...
      const fileNameWithExt = fileName.endsWith(".xlsx")
        ? fileName
        : `${fileName}.xlsx`;
