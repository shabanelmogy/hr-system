 ...
 * Mock generation is intentionally local and never persists or submits data.
 */
export function getNextMockSample<T>(
  ...
  if (samples.length === 0) {
    throw new Error("At least one mock sample is required.");
  }

  if (usedIndexes.size >= samples.length) usedIndexes.clear();

  const startIndex = Math.min(
    Math.max(Math.floor(random() * samples.length), 0),
    samples.length - 1,
  );
