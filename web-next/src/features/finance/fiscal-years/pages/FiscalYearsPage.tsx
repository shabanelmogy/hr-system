...
export default function FiscalYearsPage() {
  ...
  const lifecycle = useChangeFiscalYearLifecycle({ onSuccess: item => { showToast.success(t("fiscalYears.messages.lifecycleChanged", { status: t(`fiscalYears.status.${["", "draft", "open", "closing", "closed", "locked"][item.status]}`) })); setDialog(null); setLifecycleAction(null); }, onError: error => fail(error, "fiscalYears.messages.lifecycleError") });
