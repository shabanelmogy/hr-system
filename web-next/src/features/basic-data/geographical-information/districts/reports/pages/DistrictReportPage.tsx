...
export default function DistrictReportPage({ showFilterBar = true }: DistrictReportPageProps) {
  const { t, i18n } = useTranslation();
  const [selectedReportId, setSelectedReportId] = useState<string | null>(null);
  const reportsQuery = useQuery({
    queryKey: ["crystal-reports", "published", "districts"],
    queryFn: () => crystalReportService.listPublished("districts"),
    staleTime: 5 * 60_000,
  });
  ...
  if (!selectedReport) {
    return (
      ...
      >
        {reportsQuery.isError ? t("districts.reportCatalogError") : t("districts.reportUnavailable")}
      </Alert>
    );
  }
        ...
        <>
          {showFilterBar ? (
            <>
              ...
              <MyTextField
                fieldName="StateAr"
                value={String(currentParams.StateAr ?? "")}
                label={t("districts.stateNameAr")}
                onChange={(event) => updateSearchParams({ StateAr: event.target.value })}
                onClear={() => updateSearchParams({ StateAr: null })}
                appearance="plain"
                margin="none"
                showCounter={false}
                clearButtonAriaLabel={t("general.clearSearch")}
              ...
              <MyTextField
                fieldName="StateEn"
                value={String(currentParams.StateEn ?? "")}
                label={t("districts.stateNameEn")}
                onChange={(event) => updateSearchParams({ StateEn: event.target.value })}
                onClear={() => updateSearchParams({ StateEn: null })}
                appearance="plain"
                margin="none"
                showCounter={false}
                clearButtonAriaLabel={t("general.clearSearch")}
