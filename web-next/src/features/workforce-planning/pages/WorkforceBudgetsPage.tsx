...
export default function WorkforceBudgetsPage() {
  ...
  const fiscalOptions = useMemo(() => (fiscalYears.data ?? []).map(year => ({ id: year.id, label: `${year.code} — ${i18n.language.startsWith("ar") ? year.nameAr : year.nameEn}` })), [fiscalYears.data, i18n.language]);
  const planOptions = useMemo(() => (sourcePlans.data?.items ?? []).map(plan => ({ id: plan.id, label: `${plan.planCode} — ${i18n.language.startsWith("ar") ? plan.titleAr : plan.titleEn}` })), [sourcePlans.data, i18n.language]);
