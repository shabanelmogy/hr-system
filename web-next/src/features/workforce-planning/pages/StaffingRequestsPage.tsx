...
import { Add, Cancel, CheckCircle, Close, Send, Undo, Visibility } from "@mui/icons-material";
...
export default function StaffingRequestsPage() {
  ...
  const select = (item: StaffingRequestListItem, next: Dialog) => { setSelected(item); setReason(""); setCloseReason(item.remainingToHire === 0 ? 1 : item.remainingToHire < item.requestedHeadcount ? 3 : 2); setDialog(next); };
  ...
  const actions = (item: StaffingRequestListItem): CardActionItem[] => {
