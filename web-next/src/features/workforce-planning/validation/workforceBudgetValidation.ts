...
export const getWorkforceBudgetSchema = (t: TFunction) => {
  const periodAllocation = z.object({
    fiscalPeriodId: z.coerce.number().int().positive(t("workforceBudget.validation.period")),
    targetHeadcount: z.coerce.number().int().nonnegative(t("workforceBudget.validation.nonNegative")),
    allocatedSalaryCost: z.coerce.number().nonnegative(t("workforceBudget.validation.nonNegative")),
    allocatedRecruitmentCost: z.coerce.number().nonnegative(t("workforceBudget.validation.nonNegative")),
  });
  const line = z.object({
    workforcePlanLineId: z.coerce.number().int().positive(),
    authorizedHeadcount: z.coerce.number().int().nonnegative(t("workforceBudget.validation.nonNegative")),
    allocatedSalaryBudget: z.coerce.number().nonnegative(t("workforceBudget.validation.nonNegative")),
    allocatedRecruitmentBudget: z.coerce.number().nonnegative(t("workforceBudget.validation.nonNegative")),
    periodAllocations: z.array(periodAllocation).min(1, t("workforceBudget.validation.period")),
  }).superRefine((value, context) => {
    const seen = new Set<number>();
    value.periodAllocations.forEach((allocation, index) => {
      if (seen.has(allocation.fiscalPeriodId)) context.addIssue({ code: "custom", path: ["periodAllocations", index, "fiscalPeriodId"], message: t("workforceBudget.validation.duplicatePeriod") });
      seen.add(allocation.fiscalPeriodId);
    });
  ...
  return z.object({
    ...
    workforcePlanId: z.coerce.number().int().positive(t("workforceBudget.validation.plan")),
...
export type WorkforceBudgetFormValues = z.infer<ReturnType<typeof getWorkforceBudgetSchema>>;
