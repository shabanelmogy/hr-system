...
- `AccountingDbContext` owns the short `acc` schema and its migrations history
  boundary; the initial migration is empty because no accounting entities exist
  yet.
- No Accounting controllers, public API contracts, Next.js routes, mobile
  screens, ledger entities, or posting workflows are implemented yet.

Those absent surfaces are intentionally **not implemented**, not hidden behind
placeholders. Start with [ARCHITECTURE.md](ARCHITECTURE.md), then use
...
[phases/README.md](phases/README.md); every phase is currently Planned / Not
started and is not runtime evidence.
