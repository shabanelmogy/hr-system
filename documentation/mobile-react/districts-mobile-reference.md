...
`DistrictReportView` uses the Platform Super Admin global Crystal catalog/render boundary for entity key `districts`; it does not require tenant `CrystalReports:View`. It localizes catalog names from SummaryInfo Title/Subject with the manager display-name fallback and renders only the selected source ID, expected SHA-256, `ar|en`, and nonblank approved filters `NameAr`, `NameEn`, `StateAr`, and `StateEn` through the authenticated ERP/Reporting API.
