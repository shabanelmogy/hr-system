...
| `accounting-core-gl` | Accounting Core General Ledger | Accounting | Draft | `documentation/plans/business/accounting-core-gl/PLAN.md` | Accounting Core V1 | Overall plan/release remains Draft/G4-gated. Slice 1 has current v2 child contracts and approved UI patterns, but only the roadmap's `fiscal-years` step is Active; every later child remains Queued until ordered live verification/closure. Named masters require end-to-end `NameAr`/`NameEn` parity. |
