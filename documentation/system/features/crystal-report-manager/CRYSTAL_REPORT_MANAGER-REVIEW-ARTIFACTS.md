...
| E-08 | Mobile global geography screens use the Super Admin global Crystal catalog/render endpoints, while tenant/company screens continue using the managed tenant catalog; both validate catalog rows with Zod and persist only validated PDF bytes in temporary cache. | `mobile-react/src/platform/reporting`; geographic `*ReportView.tsx` components; `mobile-react/src/platform/auth/presentation/rbac/permissions.ts` | Mobile typecheck, architecture check, transport/authorization tests, and focused Jest suite |
