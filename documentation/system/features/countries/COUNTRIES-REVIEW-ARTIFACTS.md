...
| R-05 | Report behavior is connected | Super Admin global Crystal catalog/render boundary with ReferenceData-owned dataset | Global Report view through shared managed Crystal viewer | Global Report view through shared managed Crystal client and PDF handling | Verified; tenant Reporting is not used for global geography |
...
| E-API-07 | Global geography reports are restricted to Super Admin, use the deployment-owned catalog with exact source hash validation, and resolve data through the ReferenceData reporting contract | `GlobalCrystalReportsController.cs`, global Crystal query handlers, `ReferenceDataCrystalReportProviders.cs`, and `GlobalCrystalReportQueryTests.cs` |
