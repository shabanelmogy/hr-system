...
| Permissions and read-only | `AttendanceDevices:View/Create/Edit/SetStatus/EditCredentials/Detect/Test/PullUsers/PullAttendance/ViewRaw` plus `AttendanceAgents:Create`. Mutations fail closed for a missing exact permission or application read-only state. All connector operations are read-only against hardware. |
