...
Desktop uses Grid, Cards, Chart, Report, and Import; mobile uses Table, Cards, Chart, Report, and Import. Grid versus Table and device-specific PDF open/share are intentional platform differences. Both chart visualizations use the loaded server page and display the server total separately. Both Report views use the same managed entity/filter contract, and both Import views enforce the same headers, limits, dependency resolution, duplicate scope, permissions, atomic envelope, and uncertain-result reconciliation.
...
The approved five-view contract is implemented on web and mobile. Deployment must still upload/publish a compatible District `.rpt` version and grant managed Run access before either client receives a runnable catalog item.
