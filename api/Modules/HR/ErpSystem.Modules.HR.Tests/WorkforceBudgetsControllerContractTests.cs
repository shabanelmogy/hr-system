...
public sealed class WorkforceBudgetsControllerContractTests
{
    ...
    public void Controllers_AreThinTenantMemberSurfacesWithExactRoutesAndAuthorization()
    {
        ...
        AssertBudgetRoute<HttpGetAttribute>(nameof(WorkforceBudgetsController.GetPage), null, HrPermissions.ViewWorkforceBudgets);
        ...
        Assert.Equal(HrPermissions.ViewPositionEnvelopes, page.GetCustomAttribute<HasPermissionAttribute>()?.Policy);
        ...
        Assert.Equal(HrPermissions.ViewPositionEnvelopes, byId.GetCustomAttribute<HasPermissionAttribute>()?.Policy);
    }
        ...
        where TAttribute : HttpMethodAttribute
    {
        var method = typeof(WorkforceBudgetsController).GetMethod(action)!;
        Assert.Equal(template, Assert.Single(method.GetCustomAttributes<TAttribute>()).Template);
        Assert.Equal(PlatformRoleNames.Admin, method.GetCustomAttribute<AuthorizeAttribute>()?.Roles);
