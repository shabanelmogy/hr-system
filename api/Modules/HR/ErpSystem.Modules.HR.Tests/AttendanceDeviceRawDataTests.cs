...
public sealed class AttendanceDeviceRawDataTests
{
    ...
    public void IdempotencyKey_IsStableForSameRawEvent_AndChangesForDistinctRawEvent()
    {
        ...
        IAttendanceEventKeyGenerator keys = new AttendanceEventKeyGenerator();

        Assert.Equal(keys.Create("zkteco-com", punch), keys.Create("zkteco-com", same));
        Assert.NotEqual(keys.Create("zkteco-com", punch), keys.Create("zkteco-com", different));
    }
    ...
    {
        var fields = typeof(ErpSystem.Modules.HR.Domain.Attendance.Devices.Entities.RawAttendancePunch)
