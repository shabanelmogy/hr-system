...
using ErpSystem.Modules.HR.Presentation.Features.Attendance.Devices.V1;
...
public sealed class AttendanceDevicesArchitectureTests
{
    ...
    [Fact]
    public void Mapping_UsesMapsterFlatteningWithoutExplicitBranchOrAgentRules()
    {
        var config = new TypeAdapterConfig();
        new AttendanceDeviceMapping().Register(config);
        var device = new AttendanceDevice
        {
            ...
            BranchId = 3,
            Branch = new Branch("BR-3", "Port Said", "بورسعيد", "Africa/Cairo", new DateOnly(2026, 1, 1)),
            AttendanceAgentId = Guid.NewGuid(),
            AttendanceAgent = new AttendanceAgent { Name = "Agent A" }
        };

        var response = device.Adapt<AttendanceDeviceResponse>(config);

        Assert.Equal("Port Said", response.BranchNameEn);
        Assert.Equal("بورسعيد", response.BranchNameAr);
        Assert.Equal("Agent A", response.AttendanceAgentName);
    }
    ...
    public void LegacyMonolithicDeviceService_IsNotPresent()
    {
        Assert.Null(typeof(AttendanceDeviceStore).Assembly.GetType(
            "ErpSystem.Modules.HR.Infrastructure.Features.Attendance.Devices.Services.AttendanceDeviceService"));
    }
