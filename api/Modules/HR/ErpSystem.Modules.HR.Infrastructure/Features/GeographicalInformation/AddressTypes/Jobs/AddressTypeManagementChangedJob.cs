...
namespace ErpSystem.Modules.HR.Infrastructure.Features.GeographicalInformation.AddressTypes.Jobs;
...
public sealed class AddressTypeManagementChangedJob(INotificationPublisher notificationPublisher, IRealtimeEntityPublisher realtimePublisher)
{
    public async Task ExecuteAsync(AddressTypeChange request, CancellationToken cancellationToken)
    {
        ...
        var notification = NotificationPublishRequestFactory.Create(Permissions.ViewAddressTypes, "GeographicalInformation", nameof(AddressType), "AddressTypes", request.Action, parameters, request.AddressType?.Id.ToString(CultureInfo.InvariantCulture), "/basic-data/address-types", request.ActorUserId, request.OperationId, request.TenantId, request.CompanyId);
        ...
        await realtimePublisher.PublishAsync(RealtimeChangeRequest.For<AddressType>(RealtimeAudience.ForCompanyPermission(request.TenantId, request.CompanyId, Permissions.ViewAddressTypes), request.Action, request.AddressType?.Id.ToString(CultureInfo.InvariantCulture), request.OperationId), cancellationToken);
    }
}
