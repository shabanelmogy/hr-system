...
namespace ErpSystem.Modules.HR.Infrastructure.Features.Recruitment.Services;
    ...
    TimeProvider? timeProvider = null) : IRecruitmentService
{
    ...
    private async Task<int?> ResolveActorEmployeeIdAsync(CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(_currentActor.UserId) ||
            string.IsNullOrWhiteSpace(_currentActor.TenantId) ||
            _currentActor.CompanyId is not > 0)
            return null;

        var employee = await _context.Employees
            .AsNoTracking()
            .Where(e => e.UserId == _currentActor.UserId &&
                        e.TenantId == _currentActor.TenantId &&
                        e.CompanyId == _currentActor.CompanyId.Value &&
                        !e.IsDeleted)
            .Select(e => (int?)e.Id)
            .FirstOrDefaultAsync(cancellationToken);
        return employee is > 0 ? employee : null;
    }

    private async Task<Result<int>> RequireActorEmployeeIdAsync(CancellationToken cancellationToken) =>
        (await ResolveActorEmployeeIdAsync(cancellationToken)) is int employeeId
            ? Result.Success(employeeId)
            : Result.Failure<int>(RecruitmentErrors.ActorEmployeeRequired);

    private static (int PageNumber, int PageSize) NormalizePage(int pageNumber, int pageSize) =>
        (Math.Max(1, pageNumber), Math.Clamp(pageSize, 1, PaginationRequest.MaxPageSize));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (status.HasValue)
        {
            var domainStatus = (JobRequisitionStatus)(int)status.Value;
            query = query.Where(r => r.Status == domainStatus);
        }
    ...
    public async Task<Result<JobRequisitionDto>> CreateJobRequisitionAsync(JobRequisitionMutation mutation, CancellationToken cancellationToken = default)
    {
        ...
        var actorResult = await RequireActorEmployeeIdAsync(cancellationToken);
        if (actorResult.IsFailure)
            return Result.Failure<JobRequisitionDto>(actorResult.Error);
        var requestedByEmployeeId = actorResult.Value;
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (actorResult.IsFailure)
            return Result.Failure<JobRequisitionDto>(actorResult.Error);
        var actorEmployeeId = actorResult.Value;
    ...
    public async Task<Result<JobRequisitionDto>> ApproveJobRequisitionAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        requisition.Approve(actorResult.Value, _clock.GetUtcNow());
        await _context.SaveChangesAsync(cancellationToken);
    ...
    public async Task<Result<JobRequisitionDto>> RejectJobRequisitionAsync(int id, string reason, CancellationToken cancellationToken = default)
    {
        ...
        requisition.Reject(actorResult.Value, reason, _clock.GetUtcNow());
        await _context.SaveChangesAsync(cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (status.HasValue)
        {
            var domainStatus = (JobOpeningStatus)(int)status.Value;
    ...
    public async Task<Result<JobOpeningDto>> GetJobOpeningByIdAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        var jobDesc = await _context.JobDescriptions
            .AsNoTracking()
            .Where(jd => jd.PositionId == item.PositionId && jd.Status == ErpSystem.Modules.HR.Domain.OrganizationalStructure.Enums.JobDescriptionStatus.Approved)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (status.HasValue)
        {
            var domainStatus = (JobPostingStatus)(int)status.Value;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (status.HasValue)
        {
            var domainStatus = (ApplicationStatus)(int)status.Value;
            query = query.Where(a => a.Status == domainStatus);
        }
        ...
        CancellationToken cancellationToken = default)
    {
        var actorResult = await RequireActorEmployeeIdAsync(cancellationToken);
        if (actorResult.IsFailure)
            return Result.Failure<EmploymentApplicationDto>(actorResult.Error);
        ...
        switch ((ApplicationStatus)(int)targetStatus)
        {
    ...
    {
        return await MoveApplicationStageAsync(id, ApplicationStatusFilter.Rejected, reason, cancellationToken);
    }
    ...
    {
        return await MoveApplicationStageAsync(id, ApplicationStatusFilter.Withdrawn, reason, cancellationToken);
    }
    ...
    public async Task<Result<EmploymentApplicationDto>> HireApplicationAsync(int id, HireCandidateMutation mutation, CancellationToken cancellationToken = default)
    {
        ...
        if (preApplication is null)
            return Result.Failure<EmploymentApplicationDto>(RecruitmentErrors.EmploymentApplicationNotFound);
        if (preApplication.Status == ApplicationStatus.Hired)
            return await GetApplicationByIdAsync(preApplication.Id, cancellationToken);
        if (preApplication.Status != ApplicationStatus.OfferAccepted)
    ...
    // Interviews
    // ==========================================
    private async Task<IReadOnlyList<InterviewDto>> HydrateInterviewEmployeeNamesAsync(
        IReadOnlyList<InterviewDto> interviews,
        CancellationToken cancellationToken)
    {
        var employeeIds = interviews
            .SelectMany(interview => interview.Participants.Select(participant => participant.EmployeeId)
                .Concat(interview.Evaluations.Select(evaluation => evaluation.InterviewerEmployeeId)))
            .Where(id => id > 0)
            .Distinct()
            .ToArray();
        ...
        return interviews.Select(interview => interview with
        {
            Participants = interview.Participants.Select(participant => participant with
            {
                EmployeeName = names.TryGetValue(participant.EmployeeId, out var participantName)
                    ? participantName
                    : string.Empty
            }).ToList(),
            Evaluations = interview.Evaluations.Select(evaluation => evaluation with
            {
                InterviewerName = names.TryGetValue(evaluation.InterviewerEmployeeId, out var evaluatorName)
                    ? evaluatorName
                    : string.Empty
            }).ToList()
        }).ToList();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (status.HasValue)
        {
            var domainStatus = (InterviewStatus)(int)status.Value;
            query = query.Where(i => i.Status == domainStatus);
        }
        ...
        items = (await HydrateInterviewEmployeeNamesAsync(items, cancellationToken)).ToList();
    ...
    public async Task<Result<InterviewDto>> GetInterviewByIdAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        item = (await HydrateInterviewEmployeeNamesAsync([item], cancellationToken)).Single();
    ...
    public async Task<Result<InterviewDto>> ScheduleInterviewAsync(ScheduleInterviewMutation mutation, CancellationToken cancellationToken = default)
    {
        var actorResult = await RequireActorEmployeeIdAsync(cancellationToken);
        if (actorResult.IsFailure)
            return Result.Failure<InterviewDto>(actorResult.Error);
        ...
        var leadId = mutation.LeadEmployeeId ?? actorResult.Value;
        var participantIds = (mutation.ParticipantEmployeeIds ?? [])
            .Append(leadId)
            .Distinct()
            .ToArray();
        var validParticipantCount = await _context.Employees.AsNoTracking()
            .CountAsync(employee => participantIds.Contains(employee.Id), cancellationToken);
        ...
        {
            application.ScheduleInterview(_clock.GetUtcNow(), actorResult.Value);
        }
    ...
    public async Task<Result<InterviewDto>> CompleteInterviewAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        {
            app.RecordInterviewCompleted(_clock.GetUtcNow(), actorResult.Value);
        }
    ...
    public async Task<Result<InterviewDto>> SubmitInterviewEvaluationAsync(int interviewId, SubmitInterviewEvaluationMutation mutation, CancellationToken cancellationToken = default)
    {
        ...
        var evaluatorId = actorResult.Value;
        if (interview.Participants.All(participant => participant.EmployeeId != evaluatorId))
    ...
    public async Task<Result<InterviewScorecardTemplateDto>> GetInterviewScorecardTemplateAsync(int interviewId, CancellationToken cancellationToken = default)
    {
        ...
        var jobDesc = await _context.JobDescriptions
            .AsNoTracking()
            .Where(jd => jd.PositionId == interviewData.PositionId && jd.Status == ErpSystem.Modules.HR.Domain.OrganizationalStructure.Enums.JobDescriptionStatus.Approved)
            .OrderByDescending(jd => jd.Version)
            .FirstOrDefaultAsync(cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (status.HasValue)
        {
            var domainStatus = (JobOfferStatus)(int)status.Value;
            query = query.Where(o => o.Status == domainStatus);
        }
    ...
    public async Task<Result<JobOfferDto>> IssueJobOfferAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        var actorResult = await RequireActorEmployeeIdAsync(cancellationToken);
        if (actorResult.IsFailure)
            return Result.Failure<JobOfferDto>(actorResult.Error);
        var actorUserId = _currentActor.UserId!;
        var tenantId = _currentActor.TenantId!;
        var companyId = _currentActor.CompanyId!.Value;
        var preOffer = await _context.JobOffers.AsNoTracking()
            .FirstOrDefaultAsync(item => item.Id == id, cancellationToken);
        ...
        {
            WorkforcePlanLocks.Company(tenantId, companyId),
            $"Recruitment:Applications:{tenantId}:{companyId}:{preOffer.EmploymentApplicationId}",
            $"Recruitment:Offers:{tenantId}:{companyId}:{id}"
        };
        var result = await _context.ExecuteAtomicallyAsync(
            lockResources,
            async token =>
            {
                ...
                try
                {
                    ...
                    app.RecordOfferIssued(now, actorResult.Value);
                }
                catch (DomainRuleException)
                {
                    return Result.Failure<int>(RecruitmentErrors.InvalidOperation);
                ...
                AddOfferHistory(offer, "Issued", actorUserId, previousStatus, JobOfferStatus.Issued, null);
                await _context.SaveChangesAsync(token);
                return Result.Success(offer.Id);
            }, cancellationToken);
        if (result.IsFailure)
    ...
    public async Task<Result<JobOfferDto>> AcceptJobOfferAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        var result = await _context.ExecuteAtomicallyAsync(
            ...
            async token =>
            {
                ...
                var now = _clock.GetUtcNow();
                var previousStatus = offer.Status;
                if (app.Status != ApplicationStatus.OfferIssued)
                    return Result.Failure<int>(RecruitmentErrors.InvalidOperation);
                try
                {
                    offer.Accept(now);
                ...
                AddOfferHistory(offer, "Accepted", actorUserId, previousStatus, JobOfferStatus.Accepted, null);
                await _context.SaveChangesAsync(token);
                return Result.Success(offer.Id);
            }, cancellationToken);
        if (result.IsFailure)
