...
/// <summary>
/// Legacy ASP.NET Identity/HR persistence implementation for tenant administrators.
    ...
    ITenantAdministrationPolicy administrationPolicy)
{
    ...
    public async Task<Result<TenantAdminResponse>> CreateAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var tenantIds = administrationPolicy.NormalizeTenantIds(request.TenantIds).ToArray();
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (tenants.Any(tenant => !administrationPolicy.HasAdministratorSeat(
                tenant.MaxAdmins,
                adminCounts.GetValueOrDefault(tenant.Id))))
