...
using ITenantAdministrationPolicy = ErpSystem.Modules.Platform.Contracts.Tenancy.Administration.ITenantAdministrationPolicy;
using System.Data;

namespace ErpSystem.Modules.HR.Infrastructure.Features.Tenancy.Services;

/// <summary>
/// Legacy HR persistence adapter implementation for physical tenant tables.
/// Runtime presentation reaches it through the Platform tenancy administration seam.
/// </summary>
public sealed class TenantManagementService(
    ...
    ITenantModuleEntitlementService moduleEntitlements,
    IModuleCatalogPolicy moduleCatalog,
    ITenantAdministrationPolicy administrationPolicy,
    ILogger<TenantManagementService> logger)
{
    private const string DefaultCompanyCode = "DEFAULT";
    ...
    public async Task<Result<TenantManagementResponse>> CreateAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var identifier = administrationPolicy.NormalizeIdentifier(request.Identifier);
        if (!TryParseStatus(request.SubscriptionStatus, out var status))
            return Result.Failure<TenantManagementResponse>(InvalidStatus);

        var effectiveEntitlements = request.Entitlements is null
            ? moduleCatalog.GetDefaultEntitlements()
            : request.Entitlements;
        if (!moduleCatalog.IsValidEntitlement(effectiveEntitlements, out _))
            return Result.Failure<TenantManagementResponse>(InvalidEntitlement);

        var createResult = await context.ExecuteAtomicallyAsync(
            [TenantLockResource(identifier)],
            async token =>
            {
                if (await context.Tenants.AnyAsync(
                        ...
                        token))
                {
                    return Result.Failure<TenantCreation>(DuplicateIdentifier);
                }
                ...
                await moduleEntitlements.ApplyAsync(
                    tenant.Id,
                    effectiveEntitlements.ToHrEntitlements(),
                    token);
                ...
                var responses = await BuildResponsesAsync([tenant], token);
                return Result.Success(new TenantCreation(tenant.Id, responses[0]));
            },
            cancellationToken);

        if (createResult.IsFailure)
            return Result.Failure<TenantManagementResponse>(createResult.Error);

        DispatchChange("Create", createResult.Value.TenantId);
        return Result.Success(createResult.Value.Response);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var requestedEntitlements = request.Entitlements;
        if (requestedEntitlements is not null &&
            !moduleCatalog.IsValidEntitlement(requestedEntitlements, out _))
            return Result.Failure<TenantManagementResponse>(InvalidEntitlement);
        ...
        if (!administrationPolicy.CanSetSeatLimits(
                request.MaxAdmins,
                request.MaxUsers,
                counts.AdminCount,
                counts.UserCount))
        ...
        if (requestedEntitlements is not null)
            await moduleEntitlements.ApplyAsync(
                tenant.Id,
                requestedEntitlements.ToHrEntitlements(),
                cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var companyCounts = await context.Companies
            ...
            .ToDictionaryAsync(item => item.TenantId, item => item.Count, cancellationToken);
        var now = timeProvider.GetUtcNow().UtcDateTime;
        var entitlementMap = (await Task.WhenAll(tenants.Select(async tenant =>
                (tenant.Id, Values: await moduleEntitlements.GetAsync(tenant.Id, cancellationToken)))))
            .ToDictionary(item => item.Id, item => item.Values);
        ...
        return tenants.Select(tenant =>
        {
            ...
            return new TenantManagementResponse(
                ...
                entitlementMap.GetValueOrDefault(tenant.Id, []).ToPlatformEntitlements());
        }).ToArray();
    }
    ...
    private void DispatchChange(string action, string tenantId)
    {
        ...
        catch (Exception exception)
        {
            logger.LogWarning(
                exception,
                "Tenant realtime dispatch failed after commit. Action={Action} TenantId={TenantId}",
                action,
                tenantId);
        }
    }

    private static string TenantLockResource(string identifier)
    {
        var hash = Convert.ToHexString(SHA256.HashData(
            Encoding.UTF8.GetBytes($"tenant-provisioning|{identifier.ToUpperInvariant()}")));
        return $"ErpSystem:HR:TenantProvisioning:{hash[..32]}";
    }
    ...
    private sealed record TenantCreation(string TenantId, TenantManagementResponse Response);
    ...
    private static readonly Error InvalidEntitlement =
        new(
            "Tenant.InvalidEntitlement",
            "One or more module or submodule codes are not installed.",
            ErrorType.Validation);
}
