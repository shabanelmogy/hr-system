...
public sealed class CrystalReportStore(
    ...
    IPlatformAuthorizationSource platformAuthorization) : ICrystalReportStore
{
    ...
    private async Task<IQueryable<CrystalReport>> BuildQueryAsync(
        string? entityKey, string? search, bool includeArchived, bool publishedOnly,
        CrystalReportRight? requiredRight, bool bypassAcl)
    {
        ...
        if (!bypassAcl && requiredRight.HasValue)
        {
            ...
            var roleIds = await platformAuthorization.GetUserRoleIdsAsync(userId);
    ...
    public async Task<bool> HasRightAsync(
        Guid reportId, CrystalReportRight right, CancellationToken cancellationToken)
    {
        var userId = currentActor.UserId ?? string.Empty;
        var roleIds = await platformAuthorization.GetUserRoleIdsAsync(userId, cancellationToken);
        return await context.CrystalReportRoleGrants.AsNoTracking().AnyAsync(x =>
    ...
    {
        var values = await context.CrystalReportRoleGrants.AsNoTracking()
            .Where(grant => grant.CrystalReportId == reportId && !grant.IsDeleted)
            .OrderBy(grant => grant.RoleId)
            .Select(grant => new { grant.RoleId, grant.Rights })
            .ToListAsync(cancellationToken);
        var roleNames = (await platformAuthorization.GetRolesAsync(values.Select(item => item.RoleId).ToArray(), cancellationToken))
            .ToDictionary(item => item.Id, item => item.Name, StringComparer.Ordinal);
        return values.Select(x => new CrystalReportRoleGrantResponse(
            x.RoleId, roleNames.GetValueOrDefault(x.RoleId, x.RoleId), ToRightNames(x.Rights))).ToArray();
    }

    public async Task<IReadOnlyList<CrystalReportRoleOptionResponse>> GetGrantRoleOptionsAsync(
        CancellationToken cancellationToken)
    {
        var roles = await platformAuthorization.GetAssignableRolesAsync(
            currentActor.TenantId,
            cancellationToken);
        return roles.Select(role => new CrystalReportRoleOptionResponse(role.Id, role.Name)).ToArray();
    }
        ...
        IReadOnlyCollection<string> roleIds, CancellationToken cancellationToken)
    {
        ...
        return await platformAuthorization.AreRoleIdsValidAsync(currentActor.TenantId, distinct, cancellationToken);
    }
    ...
    {
        // Grant replacement mutates the aggregate even when no scalar domain
        // property changes. Mark the root modified so its rowversion is checked
        // and advanced by this write.
