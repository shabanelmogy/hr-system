...
{
    Task<IReadOnlyList<PlatformFileMetadata>> GetAllAsync(CancellationToken cancellationToken = default);
    Task<string> UploadAsync(PlatformFileUpload file, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<Guid>> UploadManyAsync(
        IReadOnlyCollection<PlatformFileUpload> files,
        CancellationToken cancellationToken = default);
    Task UploadImageAsync(PlatformFileUpload image, CancellationToken cancellationToken = default);
    Task<PlatformFileDownload> DownloadAsync(
        string storedFileName,
        CancellationToken cancellationToken = default);
    Task<PlatformFileDownload> StreamAsync(Guid id, CancellationToken cancellationToken = default);
