...
using ErpSystem.Modules.Platform.Infrastructure.Features.Platform.Notifications.Mapping;
...
namespace ErpSystem.Modules.Platform.Infrastructure.Features.Platform.Notifications.Persistence;

public sealed class PlatformNotificationStore(
    ...
    INotificationDeliveryEffects
{
    public bool IsKnown(string permission) =>
        moduleCatalog.GetKnownPermissions().Contains(permission);
    ...
    public async Task<IReadOnlyList<NotificationRecipient>> ResolveAsync(
        ...
        await (from access in context.UserCompanyAccesses.IgnoreQueryFilters().AsNoTracking()
               ...
               where !user.IsDisabled && (!user.LockoutEnd.HasValue || user.LockoutEnd <= utcNow) && company.IsActive &&
                     ...
                     claim.ClaimType == PermissionClaimNames.Permission && claim.ClaimValue == permission &&
        ...
        return context.Notifications.Where(item =>
            ...
            (from userRole in context.UserRoles.AsNoTracking()
             ...
             where userRole.UserId == userId && !role.IsDeleted &&
                   ...
                   claim.ClaimType == PermissionClaimNames.Permission && claim.ClaimValue == item.RequiredPermission
             select claim).Any());
    }
