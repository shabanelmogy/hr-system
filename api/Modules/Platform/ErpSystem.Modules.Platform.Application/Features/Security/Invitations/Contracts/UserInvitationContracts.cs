namespace ErpSystem.Modules.Platform.Application.Features.Security.Invitations.Contracts;
...
public sealed class AcceptUserInvitationRequestValidator : AbstractValidator<AcceptUserInvitationRequest>
{
    public AcceptUserInvitationRequestValidator()
    {
        ...
        RuleFor(request => request.Password).Cascade(CascadeMode.Stop).Trimmed().NotEmpty().Matches(ValidationPatterns.Password).Length(8, 50);
    }
}
