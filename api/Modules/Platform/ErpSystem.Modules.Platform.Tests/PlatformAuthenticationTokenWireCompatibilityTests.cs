...
public sealed class PlatformAuthenticationTokenWireCompatibilityTests
{
    ...
    [Fact]
    public void RealtimeToken_PreservesAuthorizationClaimsAndExcludesProfileOnlyClaims()
    {
    ...
    [Fact]
    public void PlatformInfrastructure_PreviewBlankSigningKeyUsesDifferentEphemeralKeysPerProvider()
    {
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["ASPNETCORE_ENVIRONMENT"] = Environments.Production,
                ["DeploymentValidation:EnforceProductionReadiness"] = "false",
        ...
        firstServices.AddPlatformInfrastructure(configuration);
        using var firstProvider = firstServices.BuildServiceProvider();
        var first = firstProvider.GetRequiredService<AuthenticationTokenOptions>();

        var secondServices = new ServiceCollection();
        secondServices.AddPlatformInfrastructure(configuration);
        using var secondProvider = secondServices.BuildServiceProvider();
        var second = secondProvider.GetRequiredService<AuthenticationTokenOptions>();

        Assert.NotEqual(first.Key, second.Key);
        Assert.True(Convert.FromBase64String(first.Key).Length >= 64);
        Assert.True(Convert.FromBase64String(second.Key).Length >= 64);
    }

    [Fact]
    public void PlatformInfrastructure_PreviewDoesNotReplaceExplicitPlaceholderSigningKey()
    {
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ...
                ["ConnectionStrings:DefaultConnection"] =
                    "Server=(local);Database=unused;Trusted_Connection=True;TrustServerCertificate=True",
                ["JwtOptions:Key"] = "YOUR_JWT_SECRET",
                ["JwtOptions:Issuer"] = "Issuer",
                ["JwtOptions:Audience"] = "Audience"
            })
            .Build();
        ...
        var exception = Assert.Throws<OptionsValidationException>(() =>
            _ = provider.GetRequiredService<IOptions<AuthenticationTokenOptions>>().Value);
    ...
    [Fact]
    public void PlatformInfrastructure_MissingSigningKey_FailsTokenOptionsValidation()
    {
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
    ...
    public void PlatformInfrastructure_ProductionRejectsKnownPlaceholderSigningKeys(string key)
    {
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
