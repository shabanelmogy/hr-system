...
public sealed class PlatformAuthorizationOwnershipTests
{
    ...
    public void HrPresentationAttributes_AreCompatibilityAliasesOverSharedPrimitives()
    {
        Assert.True(typeof(HasPermissionAttribute).IsAssignableFrom(
            typeof(ErpSystem.BuildingBlocks.Authorization.HasPermissionAttribute)));
        Assert.True(typeof(TenantMemberAttribute).IsAssignableFrom(
            typeof(ErpSystem.BuildingBlocks.Authorization.TenantMemberAttribute)));
        Assert.Equal(
            "TenantMember",
            new ErpSystem.BuildingBlocks.Authorization.TenantMemberAttribute().Policy);
    }
    ...
    public async Task PlatformPermission_WithClaim_SucceedsWithoutTenantEntitlement()
    {
        ...
        var context = CreateContext(PlatformPermissions.ViewHangfireDashboard);
    ...
    public async Task PlatformPermission_RemainsClaimOnlyForSuperAdminWithoutTenantScope()
    {
        ...
        var context = CreateContext(
            PlatformPermissions.ViewHangfireDashboard,
    ...
    private sealed class TestModuleCatalogPolicy : IModuleCatalogPolicy
    {
        ...
        public IReadOnlySet<string> GetKnownPermissions() =>
            new HashSet<string>(["Orders:View", PlatformPermissions.ViewHangfireDashboard], StringComparer.Ordinal);

        public IReadOnlySet<string> GetTenantAssignablePermissions() =>
            new HashSet<string>(["Orders:View"], StringComparer.Ordinal);
            ...
            out ModulePermissionCatalogItem resolvedPermission)
        {
            ...
            if (permission == PlatformPermissions.ViewHangfireDashboard)
            {
                resolvedPermission = new ModulePermissionCatalogItem(
                    permission,
                    "platform",
