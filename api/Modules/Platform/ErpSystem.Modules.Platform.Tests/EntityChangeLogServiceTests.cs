...
public sealed class EntityChangeLogServiceTests
{
    ...
    [Fact]
    public void ApplicationOwnership_IsPlatformOwnedAndPersistenceAdapterExposesOnlyPersistencePorts()
    {
        ...
        Assert.Equal("ErpSystem.Modules.Platform.Application", typeof(IEntityChangeLogQueryStore).Assembly.GetName().Name);
        Assert.Same(typeof(IEntityChangeLogQueryStore).Assembly, typeof(EntityChangeLogQueryRecord).Assembly);
        ...
        Assert.Equal(
            ...
            typeof(EntityChangeLogPersistenceAdapter).Assembly.GetName().Name);

        var publicContractTypes = typeof(IEntityChangeLogService).Assembly.GetTypes()
            .Where(type => string.Equals(
                type.Namespace,
                "ErpSystem.Modules.Platform.Contracts.EntityChangeLogs",
                StringComparison.Ordinal))
            .ToArray();
        Assert.DoesNotContain(typeof(IEntityChangeLogQueryStore), publicContractTypes);
        Assert.DoesNotContain(typeof(EntityChangeLogQueryRecord), publicContractTypes);
    }
    ...
    [Fact]
    public async Task PlatformPersistenceAdapter_PreservesPhysicalTableScopeAndIdentityLookup()
    {
        ...
        Assert.Equal(PlatformDbContext.Schema, context.Model.FindEntityType(typeof(EntityChangeLog))!.GetSchema());
