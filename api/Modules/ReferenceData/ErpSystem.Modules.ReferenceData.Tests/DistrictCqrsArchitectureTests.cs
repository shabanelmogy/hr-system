...
using ErpSystem.Modules.ReferenceData.Presentation.Features.GeographicalInformation.Districts.V1;
...
public sealed class DistrictCqrsArchitectureTests
{
    ...
    public void Controller_ExposesCanonicalRoutesPermissionsAndTransportShapes()
    {
        ...
        Assert.Equal(ReferenceDataPermissions.CreateDistricts, bulkCreate.GetCustomAttribute<HasPermissionAttribute>()?.Policy);
        Assert.Contains(
    ...
    public void MappingConfiguration_NormalizesOnlyMutableDistrictFields()
    {
        ...
        var district = new CreateDistrictCommand("  القاهرة  ", "  Cairo  ", " cai ", 7)
            .Adapt<ErpSystem.Modules.ReferenceData.Domain.GeographicalInformation.Districts.Entities.District>(config);
