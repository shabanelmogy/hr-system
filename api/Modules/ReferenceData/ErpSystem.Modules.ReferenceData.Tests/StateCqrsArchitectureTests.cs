...
using ErpSystem.Modules.ReferenceData.Presentation.Features.GeographicalInformation.States.V1;
...
public sealed class StateCqrsArchitectureTests
{
    ...
    public void Controller_ExposesCanonicalRoutesPermissionsAndTransportShapes()
    {
        ...
        Assert.Equal(ReferenceDataPermissions.CreateStates, bulkCreate.GetCustomAttribute<HasPermissionAttribute>()?.Policy);
        Assert.Contains(
    ...
    public void MappingConfiguration_NormalizesOnlyMutableStateFields()
    {
        ...
        var state = new CreateStateCommand("  القاهرة  ", "  Cairo  ", " cai ", 7)
            .Adapt<ErpSystem.Modules.ReferenceData.Domain.GeographicalInformation.States.Entities.State>(config);
