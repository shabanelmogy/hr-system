...
using ErpSystem.Modules.ReferenceData.Application;
using ErpSystem.Modules.ReferenceData.Contracts.Authorization;
using ErpSystem.Modules.ReferenceData.Infrastructure;
using ErpSystem.Modules.ReferenceData.Presentation;
using ErpSystem.Modules.ReferenceData.Infrastructure.Seeds;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Configuration;
using Microsoft.Extensions.DependencyInjection;

namespace ErpSystem.Modules.ReferenceData;

/// <summary>Bootstrap composition root for the ReferenceData bounded context.</summary>
public sealed class ReferenceDataModule : IModule
{
    public string Name => "ReferenceData";
    public ModuleDefinition Definition => new("reference-data", "ReferenceData",
    [
        new SubmoduleDefinition(
            "addresses",
            "Addresses",
            ReferenceDataPermissions.TenantReferenceData),
        new SubmoduleDefinition(
            "geography",
            "Geography",
            ReferenceDataPermissions.GlobalGeography,
    ...
    {
        services.AddReferenceDataApplication();
        services.AddReferenceDataInfrastructure(configuration);
        services.AddReferenceDataPresentation();
    }
    ...
    internal static string EnsureSchemaSql =>
        $"IF SCHEMA_ID(N'{ReferenceDataDbContext.Schema}') IS NULL EXEC(N'CREATE SCHEMA [{ReferenceDataDbContext.Schema}]');";
    ...
    public async Task MigrateAsync(IServiceProvider services, CancellationToken cancellationToken = default)
    {
        ...
        await EgyptGeographicSeed.SeedAsync(db, cancellationToken).ConfigureAwait(false);
    }
