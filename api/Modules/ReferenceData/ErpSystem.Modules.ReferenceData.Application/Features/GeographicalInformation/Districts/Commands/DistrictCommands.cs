...
namespace ErpSystem.Modules.ReferenceData.Application.Features.GeographicalInformation.Districts.Commands;
...
public class DistrictMutationValidator<TMutation> : AbstractValidator<TMutation> where TMutation : DistrictMutation
{
    public DistrictMutationValidator(IStringLocalizer<CreateDistrictRequest> localizer)
    {
        ...
        RuleFor(district => district.Code)
            .Trimmed().NotEmpty().WithName(ValidationMessageKeys.Code).WithMessage(localizer[ValidationMessageKeys.Required])
            .Length(2, 10).WithMessage(localizer[ValidationMessageKeys.MaxLengthError])
            .Matches(ValidationPatterns.StateCode).WithMessage(localizer[ValidationMessageKeys.InvalidValues]);
        RuleFor(district => district.StateId).GreaterThan(0).WithName(ValidationMessageKeys.State).WithMessage(localizer[ValidationMessageKeys.Required]);
    }
}
        ...
        while (true)
        {
            ...
            var result = await unitOfWork.ExecuteAtomicallyAsync(
                ...
                async token =>
                {
                    ...
                    await districtAuditTrail.RecordUpdateAsync(district, updatedDistrict, token);
        ...
        while (true)
        {
            ...
            var result = await unitOfWork.ExecuteAtomicallyAsync(
                ...
                async token =>
                {
                    ...
                    district.DeletedByPc = currentActor.MachineName;
