...
namespace ErpSystem.Modules.ReferenceData.Application.Features.GeographicalInformation.States.Commands;
...
public class StateMutationValidator<TMutation> : AbstractValidator<TMutation> where TMutation : StateMutation
{
    public StateMutationValidator(IStringLocalizer<CreateStateRequest> localizer)
    {
        ...
        RuleFor(state => state.CountryId).GreaterThan(0).WithName(ValidationMessageKeys.Country).WithMessage(localizer[ValidationMessageKeys.Required]);
    }
}
        ...
        while (true)
        {
            ...
            var result = await unitOfWork.ExecuteAtomicallyAsync(
                ...
                async token =>
                {
                    ...
                    await stateAuditTrail.RecordUpdateAsync(state, updatedState, token);
    ...
    public async Task<Result> Handle(ArchiveStateCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = await unitOfWork.ExecuteAtomicallyAsync(
            ...
            async token =>
            {
                ...
                state.DeletedByPc = currentActor.MachineName;
