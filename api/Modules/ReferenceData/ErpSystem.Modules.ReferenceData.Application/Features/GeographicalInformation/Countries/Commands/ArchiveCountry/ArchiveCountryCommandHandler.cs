...
namespace ErpSystem.Modules.ReferenceData.Application.Features.GeographicalInformation.Countries.Commands.ArchiveCountry;
    ...
    public async Task<Result> Handle(ArchiveCountryCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = await unitOfWork.ExecuteAtomicallyAsync(
            ...
            async token =>
            {
                ...
                if (await countryWriteStore.HasCompanyUsageAsync(country.Id, token))
                    return Result.Failure(countryErrors.CountryInUseByCompany);

                country.IsDeleted = true;
                country.DeletedById = currentActor.UserId;
                country.DeletedByPc = currentActor.MachineName;
