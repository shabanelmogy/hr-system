    ...
    TypeAdapterConfig mappingConfig) : ICountryReadStore
{
    ...
    {
        // Country listing explicitly supports active/archived/all status, so the
        // store must own that policy instead of inheriting the DbContext's
        // active-only soft-delete filter.
        var query = context.Countries.IgnoreQueryFilters().AsNoTracking();
    ...
    public Task<CountryDetailResponse?> GetByIdAsync(
        int id,
        CancellationToken cancellationToken) =>
        context.Countries.IgnoreQueryFilters().AsNoTracking()
