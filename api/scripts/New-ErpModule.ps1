...
param(
    ...
    [ValidateScript({
        if ($_.Equals('HR', [StringComparison]::OrdinalIgnoreCase)) {
            return $true
        }
...
function Get-DefaultModuleSchema([string]$Name) {
    ...
    # (PointOfSale, CustomerRelationshipManagement, Inventory); only the
    ...
    switch ($Name) {
        ...
        'Pos' { return 'pos' }
        'CustomerRelationshipManagement' { return 'crm' }
        'Crm' { return 'crm' }
        'CRM' { return 'crm' }
...
# ---------- generate the six canonical projects ----------
...
Write-ModuleFile (Join-Path (Join-Path $ModuleRoot "ErpSystem.Modules.$ModuleName.Infrastructure") "$ModuleName`DbContext.cs") @"
...
public sealed class ${ModuleName}DbContext : DbContext
{
    ...
    private static readonly bool HasEntityConfigurations =
        typeof(${ModuleName}DbContext).Assembly.GetTypes().Any(type =>
            !type.IsAbstract &&
...
"@
Write-ModuleFile (Join-Path (Join-Path $ModuleRoot "ErpSystem.Modules.$ModuleName.Infrastructure") "$ModuleName`DbContextDesignFactory.cs") @"
using Microsoft.EntityFrameworkCore;
...
/// </summary>
public sealed class ${ModuleName}DbContextDesignFactory
    : IDesignTimeDbContextFactory<${ModuleName}DbContext>
{
    public ${ModuleName}DbContext CreateDbContext(string[] args)
    {
        ...
        var options = new DbContextOptionsBuilder<${ModuleName}DbContext>()
            .UseSqlServer(
                connectionString,
                sql => sql.MigrationsAssembly(typeof(${ModuleName}DbContext).Assembly.FullName)
                    .MigrationsHistoryTable("__EFMigrationsHistory", ${ModuleName}DbContext.Schema))
            .Options;
...
public static class DependencyInjection
{
    ...
    /// </summary>
    public static IServiceCollection Add${ModuleName}Presentation(this IServiceCollection services)
    {
...
Write-ModuleFile (Join-Path (Join-Path $ModuleRoot "ErpSystem.Modules.$ModuleName") "$ModuleName`Module.cs") @"
...
public sealed class ${ModuleName}Module : IModule
{
    ...
    public void RegisterServices(IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.Add${ModuleName}Presentation();
    }
    ...
    internal static string EnsureSchemaSql =>
        `$"IF SCHEMA_ID(N'{${ModuleName}DbContext.Schema}') IS NULL EXEC(N'CREATE SCHEMA [{${ModuleName}DbContext.Schema}]');";
...
if ($PSCmdlet.ShouldProcess($RegistryPath, "Register $ModuleName module")) {
    ...
    $registryLine = "new ErpSystem.Modules.$ModuleName.${ModuleName}Module(),"
...
Write-Host "Module $ModuleName scaffolded with schema '$DatabaseSchema'."
