...
{
    [SqlServerFact]
    public async Task ConcurrentPartyUpdates_RejectStaleWriterAndRollbackItsOutboxMessage()
    {
        await using var database = await SqlServerTestDatabase.CreateAsync("ContactsPartyConcurrency");
        ...
        Guid partyId;
        await using (var seed = new ContactsDbContext(options, actor))
        {
            await seed.Database.MigrateAsync();
            var created = await new CreatePartyCommandHandler(
                new PartyStore(seed),
                actor,
                new ContactsOutbox(seed),
                TimeProvider.System).Handle(
                new CreatePartyCommand("Original", null, null),
                CancellationToken.None);
            partyId = created.Id;
        }

        await using var firstContext = new ContactsDbContext(options, actor);
        await using var staleContext = new ContactsDbContext(options, actor);
        var firstStore = new PartyStore(firstContext);
        var staleStore = new PartyStore(staleContext);

        // Track revision 1 in both contexts before either writer commits.
        Assert.Equal(1, (await firstStore.GetByIdAsync(partyId))!.Revision);
        Assert.Equal(1, (await staleStore.GetByIdAsync(partyId))!.Revision);

        var firstResult = await new UpdatePartyCommandHandler(
            firstStore,
            actor,
            new ContactsOutbox(firstContext),
            TimeProvider.System).Handle(
            new UpdatePartyCommand(partyId, "First writer", null, null, 1),
            CancellationToken.None);
        Assert.Equal(2, firstResult!.Revision);

        await Assert.ThrowsAsync<DbUpdateConcurrencyException>(() =>
            new UpdatePartyCommandHandler(
                staleStore,
                actor,
                new ContactsOutbox(staleContext),
                TimeProvider.System).Handle(
                new UpdatePartyCommand(partyId, "Stale writer", null, null, 1),
                CancellationToken.None));

        await using var verify = new ContactsDbContext(options, actor);
        var persisted = await verify.Parties.AsNoTracking().SingleAsync(item => item.Id == partyId);
        Assert.Equal("First writer", persisted.DisplayName);
        Assert.Equal(2, persisted.Revision);
        Assert.Equal(2, await verify.OutboxMessages.AsNoTracking().CountAsync());
        Assert.DoesNotContain(
            await verify.OutboxMessages.AsNoTracking().Select(item => item.Payload).ToArrayAsync(),
            payload => payload.Contains("Stale writer", StringComparison.Ordinal));
    }
    ...
    public async Task PartyCrud_IsTenantCompanyScoped_AndWritesTransactionalOutboxFacts()
    {
        ...
        await using (var context = new ContactsDbContext(options, actor))
        {
            ...
            await Assert.ThrowsAsync<ConcurrencyConflictException>(() => update.Handle(
                new UpdatePartyCommand(
                    partyId,
                    "Stale update",
                    null,
    ...
    public async Task AccountingPartyConsumer_DuplicateDeliveryDoesNotDuplicateProjection_AndUpdateRefreshesIt()
    {
        ...
        var created = new PartyCreatedIntegrationEvent(
            ...
            Guid.NewGuid(), DateTimeOffset.Parse("2026-09-09T20:19:00Z"), "corr", "cause", 1);
        ...
        var updated = new PartyUpdatedIntegrationEvent(
            ...
            Guid.NewGuid(), created.OccurredOnUtc.AddMinutes(1), "corr", created.EventId.ToString("D"), 2);
        ...
        Assert.Equal(2, reference.SourceRevision);
        Assert.Equal(2, await context.InboxMessages.CountAsync());
    }

    [Fact]
    public async Task AccountingPartyProjection_UsesSourceRevisionWhenClockTimestampsArriveOutOfOrder()
    {
        var options = AccountingOptions();
        await using var context = new AccountingDbContext(options);
        var consumer = new AccountingPartyIntegrationConsumer(
            new AccountingInbox(context, new FixedTimeProvider(DateTimeOffset.Parse("2026-09-09T20:20:00Z"))),
            new AccountingPartyReferenceStore(context));
        var partyId = Guid.NewGuid();
        var occurredOn = DateTimeOffset.Parse("2026-09-09T20:19:00Z");

        Assert.Equal(IntegrationEventConsumeResult.Processed, await consumer.ConsumeAsync(
            new PartyCreatedIntegrationEvent(
                partyId, "tenant-1", 7, "Created", null, null,
                Guid.NewGuid(), occurredOn, Revision: 1)));
        var latest = new PartyUpdatedIntegrationEvent(
            partyId, "tenant-1", 7, "Revision three", null, null,
            Guid.NewGuid(), occurredOn.AddMinutes(1), Revision: 3);
        Assert.Equal(IntegrationEventConsumeResult.Processed, await consumer.ConsumeAsync(latest));

        var lateOlderUpdate = new PartyUpdatedIntegrationEvent(
            partyId, "tenant-1", 7, "Revision two", null, null,
            Guid.NewGuid(), occurredOn.AddHours(1), Revision: 2);
        Assert.Equal(IntegrationEventConsumeResult.Processed, await consumer.ConsumeAsync(lateOlderUpdate));

        context.ChangeTracker.Clear();
        var reference = await context.PartyReferences.AsNoTracking().SingleAsync();
        Assert.Equal("Revision three", reference.DisplayName);
        Assert.Equal(latest.EventId, reference.SourceEventId);
        Assert.Equal(3, reference.SourceRevision);
    }

    [Fact]
    public async Task AccountingPartyProjection_RejectsEqualRevisionAndLegacyPayloadAfterVersionedState()
    {
        var options = AccountingOptions();
        await using var context = new AccountingDbContext(options);
        var store = new AccountingPartyReferenceStore(context);
        ...
        var first = new AccountingPartyReferenceUpdate(
            partyId,
            "tenant-1",
            7,
            "Revision two",
            ...
            null,
            firstEventId,
            DateTimeOffset.Parse("2026-09-09T20:40:00Z"),
            SourceRevision: 2);

        await store.UpsertAsync(first);
        await context.SaveChangesAsync();
        await store.UpsertAsync(first with
        {
            DisplayName = "Equal revision must be ignored",
            SourceEventId = Guid.Parse("00000000-0000-0000-0000-000000000002"),
            SourceOccurredOnUtc = first.SourceOccurredOnUtc.AddHours(1)
        });
        await store.UpsertAsync(first with
        {
            DisplayName = "Legacy payload must be ignored",
            SourceRevision = 0,
            SourceEventId = Guid.Parse("00000000-0000-0000-0000-000000000003"),
            SourceOccurredOnUtc = first.SourceOccurredOnUtc.AddHours(2)
        });
        await context.SaveChangesAsync();

        context.ChangeTracker.Clear();
        var reference = await context.PartyReferences.AsNoTracking().SingleAsync();
        Assert.Equal("Revision two", reference.DisplayName);
        Assert.Equal(firstEventId, reference.SourceEventId);
        Assert.Equal(2, reference.SourceRevision);
    }

    [Fact]
    public async Task AccountingPartyProjection_RejectsNegativeSourceRevision()
    {
        await using var context = new AccountingDbContext(AccountingOptions());
        var store = new AccountingPartyReferenceStore(context);

        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => store.UpsertAsync(
            new AccountingPartyReferenceUpdate(
                Guid.NewGuid(),
                "tenant-1",
                7,
                "Invalid revision",
        ...
        Assert.Contains("complete party integration scope", exception.Message, StringComparison.Ordinal);
