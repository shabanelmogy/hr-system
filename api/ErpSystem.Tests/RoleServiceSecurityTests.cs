...
{
    [Fact]
    public async Task RoleClaims_AcceptPermissionsDeclaredByFutureBusinessModules()
    {
        var tenantId = "tenant-sec-future";
        await using var fixture = await SecurityTestFixture.CreateAsync(tenantId);
        var role = new ApplicationRole("Inventory Operator")
        {
            TenantId = tenantId,
            IsSystem = false
        };
        ...
        var before = await fixture.RoleService.GetRoleClaims(role.Id, CancellationToken.None);
        Assert.True(before.IsSuccess);
        Assert.Contains(before.Value.RoleClaims!, claim =>
            claim.DisplayValue == FuturePermissionCatalogPolicy.FuturePermission);

        var result = await fixture.RoleService.UpdateRoleClaims(
            new RoleRequest(
                role.Id,
                role.Name!,
                [new CheckBoxViewModel
                {
                    DisplayValue = FuturePermissionCatalogPolicy.FuturePermission,
                    IsSelected = true
                }]),
        ...
        Assert.Contains(claims, claim =>
            ...
            claim.Value == FuturePermissionCatalogPolicy.FuturePermission);
    }

    [Fact]
    public async Task RoleClaims_RejectPermissionsMissingFromHrAndModuleCatalogs()
    {
        var tenantId = "tenant-sec-unknown";
        await using var fixture = await SecurityTestFixture.CreateAsync(tenantId);
        var role = new ApplicationRole("Unknown Permission Role")
        {
            TenantId = tenantId,
            IsSystem = false
        };
        ...
        var result = await fixture.RoleService.UpdateRoleClaims(
            new RoleRequest(
                ...
                [new CheckBoxViewModel
                {
                    DisplayValue = "Unknown:Permission",
                    IsSelected = true
                }]),
            CancellationToken.None);

        Assert.True(result.IsFailure);
        Assert.Empty(await fixture.RoleManager.GetClaimsAsync(role));
    }
    ...
    [Fact]
    public async Task AddAsync_WhenRealtimeDispatchFails_ReturnsSuccessAfterCommit()
    {
        var tenantId = "tenant-sec-post-commit";
        await using var fixture = await SecurityTestFixture.CreateAsync(tenantId);
        fixture.RealtimeDispatcher.ThrowOnDispatch = true;

        var result = await fixture.RoleService.AddAsync(
            new RoleRequest(null, "Post Commit Role", null),
            CancellationToken.None);

        Assert.True(result.IsSuccess);
        Assert.Equal(1, await fixture.Context.Roles.CountAsync(role =>
            role.TenantId == tenantId && role.Name == "Post Commit Role"));
        Assert.Single(fixture.SecurityAudit.RecordedAudits, audit =>
            audit.Action == "RoleCreated" && audit.TargetId == result.Value.Id);
    }

    [SqlServerFact]
    public async Task AddAsync_WhenAuditFails_RollsBackRoleAndPostCommitSideEffects()
    {
        await using var database = await SqlServerTestDatabase.CreateAsync("RoleAtomicity");
        await using var fixture = await SecurityTestFixture.CreateAsync(
            "tenant-sec-atomicity",
            database.ConnectionString);
        fixture.SecurityAudit.ExceptionToThrow = new InvalidOperationException("audit failure");

        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            fixture.RoleService.AddAsync(
                new RoleRequest(null, "Atomic Role", null),
                CancellationToken.None));

        fixture.Context.ChangeTracker.Clear();
        Assert.False(await fixture.Context.Roles
            .IgnoreQueryFilters()
            .AnyAsync(role => role.TenantId == "tenant-sec-atomicity" && role.Name == "Atomic Role"));
        Assert.Empty(fixture.RealtimeDispatcher.DispatchedRequests);
    }
    ...
    private sealed class SecurityTestFixture : IAsyncDisposable
    {
        ...
        public static async Task<SecurityTestFixture> CreateAsync(
            string tenantId,
            string? sqlConnectionString = null)
        {
            ...
            fixture.RoleService = new RoleService(
                ...
                notifier,
                TimeProvider.System,
                new FuturePermissionCatalogPolicy(),
    ...
    private sealed class FuturePermissionCatalogPolicy : IModuleCatalogPolicy
    {
        public const string FuturePermission = "FutureInventory:View";
        ...
        [
            new ModuleCatalogItem(
                "future-business",
                "Future Business",
                [new ModuleSubmoduleCatalogItem("inventory", "Inventory", [FuturePermission])],
                false)
        ];
        ...
        {
            if (string.Equals(permission, FuturePermission, StringComparison.Ordinal))
            {
                moduleCode = "future-business";
                submoduleCode = "inventory";
                return true;
            }
