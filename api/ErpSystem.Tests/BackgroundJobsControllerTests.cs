...
using ErpSystem.Modules.HR.Presentation.Features.Platform.BackgroundJobs.V1;
...
{
    [Fact]
    public void BackgroundJobContracts_AreOwnedByPlatformWithoutChangingTheLegacyRouteAdapter()
    {
        Assert.Equal(
            "ErpSystem.Modules.Platform.Contracts",
            typeof(IBackgroundJobDashboardService).Assembly.GetName().Name);
        Assert.Equal(
            "ErpSystem.Modules.Platform.Contracts",
            typeof(IBackgroundJobDashboardSession).Assembly.GetName().Name);
        Assert.Null(typeof(ErpSystem.Modules.HR.Application.AssemblyReference).Assembly.GetType(
            "ErpSystem.Modules.HR.Application.Features.Platform.BackgroundJobs.Services.IBackgroundJobDashboardService"));
        Assert.Null(typeof(ErpSystem.Modules.HR.Application.AssemblyReference).Assembly.GetType(
            "ErpSystem.Modules.HR.Application.Abstractions.BackgroundJobs.IHangfireSessionAuthentication"));
    }
    ...
    public void GetDashboard_ReturnsServiceSnapshot()
    {
        ...
        var controller = new BackgroundJobsController(
            new StubDashboardService(expected),
            new HangfireSessionAuthenticationService());
