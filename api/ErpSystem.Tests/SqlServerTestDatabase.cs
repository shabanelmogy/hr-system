...
/// <summary>
/// Marks a test that requires the SQL Server instance supplied by
/// ERPSYSTEM_TEST_SQLSERVER_CONNECTION. Local runs stay deterministic when no
/// SQL Server is installed, while CI can opt into the same real-database gate.
/// </summary>
...
public sealed class SqlServerFactAttribute : FactAttribute
{
    public SqlServerFactAttribute()
    {
        if (string.IsNullOrWhiteSpace(
                Environment.GetEnvironmentVariable(SqlServerTestDatabase.ConnectionEnvironmentVariable)))
        {
            Skip =
                $"Set {SqlServerTestDatabase.ConnectionEnvironmentVariable} to run this SQL Server integration test.";
        }
    }
}
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(baseConnectionString))
        {
            throw new InvalidOperationException(
                $"{ConnectionEnvironmentVariable} must be configured before creating a SQL Server test database.");
        }
