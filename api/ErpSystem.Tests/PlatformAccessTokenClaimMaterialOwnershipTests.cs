...
{
    [Fact]
    public void ClaimMaterialOwnership_PointsFromHrToPlatformContractsOnly()
    {
        var platformContractsReferences = typeof(IAccessTokenClaimMaterialService).Assembly
        ...
        var contractTypes = typeof(IAccessTokenClaimMaterialService).Assembly.GetTypes()
            .Where(type => type.Namespace?.StartsWith(
                "ErpSystem.Modules.Platform.Contracts.Authentication.Tokens",
                StringComparison.Ordinal) == true)
            .ToArray();
        Assert.DoesNotContain(contractTypes, type => type.FullName?.Contains("ApplicationUser", StringComparison.Ordinal) == true);
        Assert.DoesNotContain(contractTypes, type => type.FullName?.Contains("Permissions", StringComparison.Ordinal) == true);

        var hrApplication = typeof(ErpSystem.Modules.HR.Application.AssemblyReference).Assembly;
        Assert.Null(hrApplication.GetType(
            "ErpSystem.Modules.HR.Application.Features.Security.Authentication.PlatformCompatibility.PlatformAccessTokenClaimMaterialSource"));
        Assert.Null(hrApplication.GetType(
            "ErpSystem.Modules.HR.Application.Features.Security.Authentication.Services.IAccessTokenClaimMaterialReadStore"));
    }
