...
public sealed class NotificationServiceTests
{
    ...
    public void BulkNotificationStateChanges_UseNotificationsUserCompanyContract(string action)
    {
        ...
        var dispatcher = new RecordingRealtimeDispatcher();
        var adapter = CreateAdapter(context, dispatcher);
        adapter.Publish(new NotificationInboxChange("tenant-1", 1, "user-1", action, null));
    ...
    {
        var adapter = CreateAdapter(context, dispatcher ?? new RecordingRealtimeDispatcher());
        var services = new ServiceCollection();
        services.AddPlatformApplication();
        services.AddSingleton<INotificationInboxStore>(adapter);
        services.AddSingleton<INotificationInboxEffects>(adapter);
        var provider = services.BuildServiceProvider();
        return new NotificationService(
            provider.GetRequiredService<INotificationInboxService>(),
            CreateErrors(),
            actor ?? new TestCurrentActor());
    }

    private static INotificationPublisher CreatePublisher(ApplicationDbContext context)
    {
        var adapter = CreateAdapter(context, new RecordingRealtimeDispatcher());
        var services = new ServiceCollection();
        services.AddPlatformApplication();
        services.AddSingleton<INotificationPermissionCatalog>(adapter);
        services.AddSingleton<INotificationRecipientResolver>(adapter);
        services.AddSingleton<INotificationPublicationStore>(adapter);
        services.AddSingleton<INotificationDeliveryEffects>(adapter);
        return services.BuildServiceProvider().GetRequiredService<INotificationPublisher>();
    }

    private static LegacyNotificationAdapter CreateAdapter(
        ApplicationDbContext context,
        RecordingRealtimeDispatcher dispatcher)
    {
        var hubClient = DispatchProxy.Create<IGeneralHubClient, NoOpHubClientProxy>();
        return new LegacyNotificationAdapter(context, dispatcher, new TestHubContext(hubClient));
    }

    private static INotificationPublicationPolicy CreatePublicationPolicy()
    {
        var services = new ServiceCollection();
        services.AddPlatformApplication();
        return services.BuildServiceProvider().GetRequiredService<INotificationPublicationPolicy>();
    }
