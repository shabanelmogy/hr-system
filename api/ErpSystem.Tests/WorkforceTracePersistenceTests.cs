...
public sealed class WorkforceTracePersistenceTests
{
    ...
    public async Task TraceEdgesAlwaysReferenceReturnedNodes_AndFinancialsCanBeRedacted()
    {
        ...
        var store = new WorkforceTraceReadStore(context);
    ...
    public async Task PlanCommitment_IsBoundedAndExcludesSupersededBudgets()
    {
        var actor = new TestActor("tenant-1", 11);
        await using var context = CreateContext(actor);
        var fiscalYear = CreateFiscalYear(4, "FY-2027", open: true);
        var plan = CreatePlan(10, fiscalYear.Id, "WP-2027");
        var currentBudget = CreateBudget(20, plan.Id, fiscalYear.Id, "WB-CURRENT", superseded: false);
        var supersededBudget = CreateBudget(21, plan.Id, fiscalYear.Id, "WB-OLD", superseded: true);
        ...
        context.AddRange(fiscalYear, plan, currentBudget, supersededBudget, currentEnvelope, supersededEnvelope);
        await context.SaveChangesAsync();
    ...
    private static FiscalYear CreateFiscalYear(int id, string code, bool open)
    {
        var year = new FiscalYear(code, "السنة", "Fiscal year", new DateOnly(2027, 1, 1), new DateOnly(2027, 12, 31), FiscalPeriodFrequency.Monthly)
        {
