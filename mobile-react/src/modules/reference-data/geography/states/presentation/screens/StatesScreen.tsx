...
import type { State, StateFilters, StateRequest, StateSearchField, StateSearchOperator, StateSortColumn } from '../../domain/models/state';
...
export function StatesScreen() {
  ...
  const { allowed: isArchiveAuthorized } = useAuthorization({ allowSuperAdmin: true, requiredPermissions: [permissions.ArchiveStates] });
  const { allowed: isRestoreAuthorized } = useAuthorization({ allowSuperAdmin: true, requiredPermissions: [permissions.RestoreStates] });
  const { allowed: canViewReports } = useManagedReportAvailability('global');
  const canCreate = isCreateAuthorized && !isReadOnly; const canEdit = isEditAuthorized && !isReadOnly; const canArchive = isArchiveAuthorized && !isReadOnly; const canRestore = isRestoreAuthorized && !isReadOnly;
  ...
  const [formMode, setFormMode] = useState<FormMode>('view'); const [selectedState, setSelectedState] = useState<State | null>(null); const [pendingAction, setPendingAction] = useState<PendingAction>(null); const [selectedIds, setSelectedIds] = useState<number[]>([]); const [flash, setFlash] = useState<AppDataTableFlash>();
  ...
  const save = useCallback(async (request: StateRequest) => { if (isReadOnly) return notifyBlockedAction(); if (!(formMode === 'edit' ? isEditAuthorized : isCreateAuthorized)) return; try { const editingId = formMode === 'edit' ? selectedState?.id ?? null : null; await saveMutation.mutateAsync({ id: editingId, request }); if (editingId !== null) setFlash((current) => ({ rowKey: editingId, token: (typeof current?.token === 'number' ? current.token : 0) + 1 })); closeForm(); showToast.success(t('states.saved')); } catch (error) { showToast.error(error, t('states.saveFailed')); } }, [closeForm, formMode, isCreateAuthorized, isEditAuthorized, isReadOnly, notifyBlockedAction, saveMutation, selectedState, t]);
  const restore = useCallback(async (state: State) => { if (isReadOnly) return notifyBlockedAction(); if (!isRestoreAuthorized) return; try { await restoreMutation.mutateAsync(state.id); showToast.success(t('states.restored')); } catch (error) { showToast.error(error, t('states.restoreFailed')); } }, [isReadOnly, isRestoreAuthorized, notifyBlockedAction, restoreMutation, t]);
  const confirmAction = useCallback(async () => { if (!pendingAction) return; if (isReadOnly) return notifyBlockedAction(); if (!isArchiveAuthorized) return; try { if (pendingAction.kind === 'bulk') { const result = await bulkArchiveMutation.mutateAsync(selectedIds); setSelectedIds([]); showToast.success(t('states.bulkArchived', { count: result.archivedCount })); } else { await archiveMutation.mutateAsync(pendingAction.state.id); setSelectedIds((ids) => ids.filter((id) => id !== pendingAction.state.id)); showToast.success(t('states.archivedSuccess')); } setPendingAction(null); } catch (error) { showToast.error(error, t('states.archiveFailed')); } }, [archiveMutation, bulkArchiveMutation, isArchiveAuthorized, isReadOnly, notifyBlockedAction, pendingAction, selectedIds, t]);
  const columns = useMemo<AppDataTableColumn<State>[]>(() => [
    ...
    { id: 'actions', header: t('states.actions'), width: 180, align: 'center', render: (state) => <View style={styles.tableActions}><AppIconButton icon="eye-outline" label={t('states.viewState')} onPress={() => openForm('view', state)} />{canEdit && !state.isDeleted ? <AppIconButton icon="create-outline" label={t('states.editState')} onPress={() => openForm('edit', state)} /> : null}{!state.isDeleted && canArchive ? <AppIconButton icon="archive-outline" label={t('states.archive')} onPress={() => setPendingAction({ kind: 'archive', state })} /> : null}{state.isDeleted && canRestore ? <AppIconButton icon="refresh-outline" label={t('states.restore')} onPress={() => void restore(state)} /> : null}</View> },
  ...
  return <AppScreen contentContainerStyle={styles.screen} edges={['left', 'right', 'bottom']} refreshControl={<RefreshControl colors={[theme.colors.primary]} onRefresh={() => void statesQuery.refetch()} refreshing={statesQuery.isRefetching} tintColor={theme.colors.primary} />}>
    <AppListScreen<State, 'table' | 'cards' | 'chart' | 'report' | 'import'> aboveViews={selectedIds.length > 0 && canArchive ? <AppButton icon="archive-outline" onPress={() => setPendingAction({ kind: 'bulk' })} variant="outline">{t('states.archiveSelected', { count: selectedIds.length })}</AppButton> : null} defaultView="table" emptyContent={<AppStateView message={t('states.empty')} state="empty" />} fillViewSelector filterControl={<StateFilterButton field={searchField} onApply={applyFilters} operator={searchOperator} status={list.state.filters.status} />} isFetching={statesQuery.isFetching} items={rows} onSearchChange={changeSearch} searchActions={canCreate ? <AppIconButton color={theme.colors.onPrimary} icon="add-outline" label={t('states.addState')} onPress={() => openForm('create', null)} size={22} style={({ pressed }) => ({ backgroundColor: theme.colors.primary, opacity: pressed ? 0.75 : 1 })} /> : null} searchPlaceholder={t('states.search')} searchValue={list.searchInput} serverPagination={{ onPageChange: changePage, onPageSizeChange: changePageSize, page: list.state.page, pageSize: list.state.pageSize, pageSizeOptions: [3, 5, 10], totalItems: statesQuery.data?.metaData.totalCount ?? 0 }} showResultCount={false} showViewLabels views={[
      ...
      ...(canViewReports ? [{ value: 'report' as const, icon: 'document-text-outline' as const, label: t('states.reportView'), paginate: false, renderWhenEmpty: true, render: () => <StateReportView /> }] : []),
