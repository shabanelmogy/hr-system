...
import type { District, DistrictFilters, DistrictRequest, DistrictSearchField, DistrictSearchOperator, DistrictSortColumn } from '../../domain/models/district';
...
export function DistrictsScreen() {
  ...
  const { allowed: isArchiveAuthorized } = useAuthorization({ allowSuperAdmin: true, requiredPermissions: [permissions.ArchiveDistricts] });
  const { allowed: isRestoreAuthorized } = useAuthorization({ allowSuperAdmin: true, requiredPermissions: [permissions.RestoreDistricts] });
  ...
  const [formMode, setFormMode] = useState<FormMode>('view'); const [selectedDistrict, setSelectedDistrict] = useState<District | null>(null); const [pendingAction, setPendingAction] = useState<PendingAction>(null); const [selectedIds, setSelectedIds] = useState<number[]>([]); const [flash, setFlash] = useState<AppDataTableFlash>();
  ...
  const save = useCallback(async (request: DistrictRequest) => { if (isReadOnly) return notifyBlockedAction(); if (!(formMode === 'edit' ? isEditAuthorized : isCreateAuthorized)) return; try { const editingId = formMode === 'edit' ? selectedDistrict?.id ?? null : null; await saveMutation.mutateAsync({ id: editingId, request }); if (editingId !== null) setFlash((current) => ({ rowKey: editingId, token: (typeof current?.token === 'number' ? current.token : 0) + 1 })); closeForm(); showToast.success(t('districts.saved')); } catch (error) { showToast.error(error, t('districts.saveFailed')); } }, [closeForm, formMode, isCreateAuthorized, isEditAuthorized, isReadOnly, notifyBlockedAction, saveMutation, selectedDistrict, t]);
  const restore = useCallback(async (district: District) => { if (isReadOnly) return notifyBlockedAction(); if (!isRestoreAuthorized) return; try { await restoreMutation.mutateAsync(district.id); showToast.success(t('districts.restored')); } catch (error) { showToast.error(error, t('districts.restoreFailed')); } }, [isReadOnly, isRestoreAuthorized, notifyBlockedAction, restoreMutation, t]);
  const confirmAction = useCallback(async () => { if (!pendingAction) return; if (isReadOnly) return notifyBlockedAction(); if (!isArchiveAuthorized) return; try { if (pendingAction.kind === 'bulk') { const result = await bulkArchiveMutation.mutateAsync(selectedIds); setSelectedIds([]); showToast.success(t('districts.bulkArchived', { count: result.archivedCount })); } else { await archiveMutation.mutateAsync(pendingAction.district.id); setSelectedIds((ids) => ids.filter((id) => id !== pendingAction.district.id)); showToast.success(t('districts.archivedSuccess')); } setPendingAction(null); } catch (error) { showToast.error(error, t('districts.archiveFailed')); } }, [archiveMutation, bulkArchiveMutation, isArchiveAuthorized, isReadOnly, notifyBlockedAction, pendingAction, selectedIds, t]);
