...
import type { Country, CountryFilters, CountryRequest, CountrySearchField, CountrySearchOperator, CountrySortColumn } from '../../domain/models/country';
...
export function CountriesScreen() {
  ...
  const centralOfflineReadAllowed = offlinePolicy.loaded
    && offlinePolicy.canReadOffline(OFFLINE_CAPABILITY_IDS.countriesRead);
  const offlineReadEnabled = centralOfflineReadAllowed
  ...
  const countryPage = countriesQuery.data?.value;
  const rows = countryPage?.items ?? [];
  ...
  const offlineControls = (
    <View style={styles.offlineControls}>
      <AppSwitchField
        disabled={!offlinePreferences.loaded || !offlinePolicy.loaded || !centralOfflineReadAllowed}
        description={t('countries.offline.description')}
        icon="cloud-offline-outline"
        label={t('countries.offline.cachedReads')}
        onValueChange={(enabled) => offlinePreferences.setOfflineReadEnabled('countries', enabled)}
        value={offlineReadEnabled}
      />
      {!centralOfflineReadAllowed && offlinePolicy.loaded ? (
        <AppText color="muted" variant="caption">
          {t('offlineOperations.countriesPreferenceBlocked')}
        </AppText>
      ) : null}
      <AppText color="muted" variant="caption">
        {connectivity.isOnline
          ? t('countries.offline.online')
          : countriesQuery.data?.source === 'cache'
            ? t('countries.offline.cached')
            : t('countries.offline.offline')}
        {countriesQuery.data?.cachedAt
          ? ` · ${t('countries.offline.lastUpdated', { date: formatDateTime(countriesQuery.data.cachedAt, i18n.language) })}`
          : ''}
      </AppText>
    </View>
  );
  if (!offlinePreferences.loaded || !offlinePolicy.loaded || countriesQuery.isLoading) {
    return <AppScreen edges={['left', 'right', 'bottom']}>{offlineControls}<AppStateView state="loading" /></AppScreen>;
  }
  if (countriesQuery.error) {
    return <AppScreen edges={['left', 'right', 'bottom']}>{offlineControls}<AppStateView message={errorMessage(countriesQuery.error, t('countries.loadFailed'))} onRetry={() => void countriesQuery.refetch()} state="error" /></AppScreen>;
  }
  ...
  return <AppScreen contentContainerStyle={styles.screen} edges={['left', 'right', 'bottom']} refreshControl={<RefreshControl colors={[theme.colors.primary]} onRefresh={() => void countriesQuery.refetch()} refreshing={countriesQuery.isRefetching} tintColor={theme.colors.primary} />}>
    ...
    <AppListScreen<Country, 'table' | 'cards' | 'chart' | 'report' | 'import'>
      ...
      serverPagination={{ onPageChange: changePage, onPageSizeChange: changePageSize, page: list.state.page, pageSize: list.state.pageSize, pageSizeOptions: [3, 5, 10], totalItems: countryPage?.metaData.totalCount ?? 0 }}
...
const styles = StyleSheet.create({ screen: { gap: 12, paddingVertical: 8 }, offlineControls: { gap: 6 }, tableActions: { flexDirection: 'row', justifyContent: 'center', gap: 4 }, cards: { gap: 6 } });
