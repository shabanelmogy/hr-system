...
export function WorkforcePlansScreen() {
  ...
  return <AppScreen edges={['left', 'right', 'bottom']} contentContainerStyle={styles.screen} refreshControl={<RefreshControl refreshing={query.isRefetching} onRefresh={() => void query.refetch()} tintColor={theme.colors.primary} colors={[theme.colors.primary]} />}>
    ...
    {formOpen ? <WorkforcePlanForm key={`${formMode}-${selected?.id ?? 'new'}-${effectiveDetail?.rowVersion ?? 'pending'}`} item={effectiveDetail} draftRequest={localDraft.data?.draft.request ?? null} draftStatus={localDraft.data?.status ?? null} revisions={revisions.data ?? []} mode={formMode} loading={create.isPending || update.isPending || discardLocalDraft.isPending} detailLoading={formMode !== 'create' && !effectiveDetail && !details.error} detailError={!effectiveDetail && formMode === 'edit' && details.error ? t('workforcePlanning.offline.requiresBase') : details.error && !localDraft.data ? errorMessage(details.error, t('workforcePlanning.messages.fetchError')) : null} onDiscardDraft={selected && localDraft.data && ['conflict', 'uncertain'].includes(localDraft.data.status ?? '') ? () => { void discardLocalDraft.mutateAsync(selected.id).then(() => details.refetch()); } : undefined} onRetryDetail={() => void details.refetch()} onClose={closeForm} onSave={save} /> : null}
