...
export const useUpdateWorkforcePlan = () => {
  ...
  const { pilot, scope, isOnline, authenticated, canExecuteOfflineCommand } = useWorkforcePlanDraftPilot();
  const { isReadOnly } = useAppReadOnly();
  const client = useQueryClient();
  return useMutation({
    networkMode: 'always',
    mutationFn: async ({ id, request, baseDetail, keepLocal }: { id: number; request: UpdateWorkforcePlanRequest; baseDetail: WorkforcePlanDetail; keepLocal?: boolean }) => {
      ...
      const value = await pilot.queue(scope, baseDetail, request, canExecuteOfflineCommand);
...
export const useWorkforcePlanDraftAutoSync = () => {
  const { pilot, scope, isOnline, authenticated, canExecuteOfflineCommand } = useWorkforcePlanDraftPilot();
  const { isReadOnly } = useAppReadOnly();
  const client = useQueryClient();
  useEffect(() => {
    if (!pilot || !scope || !canExecuteOfflineCommand || !isOnline || !authenticated || isReadOnly) return;
    let cancelled = false;
    void pilot.sync(scope, { authenticated: true, readOnly: false }).then(async () => {
      if (cancelled) return;
      await client.invalidateQueries({ queryKey: workforcePlanKeys.all });
    }).catch(() => undefined);
    return () => { cancelled = true; };
  }, [authenticated, canExecuteOfflineCommand, client, isOnline, isReadOnly, pilot, scope]);
};
