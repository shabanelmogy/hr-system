...
import { permissions } from '@/src/platform/auth';
...
let mockOnline = true;
const mockSetOfflineReadEnabled = jest.fn();
...
jest.mock('@/src/core/preferences', () => ({
  useOfflineReadPreferences: () => ({
    ...
    setOfflineReadEnabled: mockSetOfflineReadEnabled,
  }),
}));
...
jest.mock('@/src/shared/components', () => {
  ...
  AppSwitchField: ({ disabled, label, onValueChange, value }: { disabled?: boolean; label: string; onValueChange: (value: boolean) => void; value: boolean }) => (
    <Pressable disabled={disabled} onPress={() => onValueChange(!value)} testID="offline-read-toggle"><Text>{label}</Text></Pressable>
  ),
...
describe('CountriesScreen', () => {
  beforeEach(() => {
    ...
    mockUseCountries.mockReturnValue({
      data: {
        value: { items: [country], metaData: { totalCount: 1 } },
        source: 'remote',
        cachedAt: '2026-09-09T12:00:00Z',
      },
      error: null,
      isFetching: false,
      isLoading: false,
  ...
  it('allows the Countries cached-read policy to be explicitly enabled', async () => {
    mockCentralOfflineReadAllowed = true;
    await render(<CountriesScreen />);

    await fireEvent.press(screen.getByTestId('offline-read-toggle'));

    expect(mockSetOfflineReadEnabled).toHaveBeenCalledWith('countries', true);
  });

  it('does not let the device toggle expand a company policy that denies cached reads', async () => {
    await render(<CountriesScreen />);

    await fireEvent.press(screen.getByTestId('offline-read-toggle'));

    expect(mockSetOfflineReadEnabled).not.toHaveBeenCalled();
    expect(screen.getByText('offlineOperations.countriesPreferenceBlocked')).toBeTruthy();
  });

  it('shows cached mode while offline and exposes the last update timestamp', async () => {
    ...
    expect(screen.getAllByText(/countries\.offline\.cached/).length).toBeGreaterThan(0);
    expect(screen.getByText(/countries\.offline\.lastUpdated/)).toBeTruthy();
  });
});
