...
export function AccountsScreen() {
  ...
  const columns = useMemo<AppDataTableColumn<Account>[]>(() => [
    ...
    { id: 'actions', header: t('common.actions'), width: 110, align: 'center', render: item => <View style={styles.actions}><AppIconButton icon="eye-outline" label={t('common.view')} onPress={() => void openForm('view', item.id)} />{canManage && !item.isDeleted ? <AppIconButton icon="create-outline" label={t('common.edit')} onPress={() => void openForm('edit', item.id)} /> : null}</View> },
  ...
  return <AppScreen edges={['left', 'right', 'bottom']} contentContainerStyle={styles.screen} refreshControl={<RefreshControl refreshing={activeQuery.isRefetching} onRefresh={() => void activeQuery.refetch()} tintColor={theme.colors.primary} colors={[theme.colors.primary]} />}>
    ...
    {workspaceView === 'tree' ? <>
      <View style={styles.treeActions}>{canManage ? <AppIconButton icon="add-outline" label={t('coaHierarchy.accounts.actions.add')} color={theme.colors.onPrimary} onPress={() => void openForm('create')} size={22} style={({ pressed }) => ({ backgroundColor: theme.colors.primary, opacity: pressed ? 0.75 : 1 })} /> : null}</View>
      <AppHierarchicalTree<AccountTreeNode> items={treeQuery.data ?? []} getId={item => item.id} getParentId={item => item.parentAccountId} getLabel={item => `${item.nameEn} / ${item.nameAr}`} getCode={item => item.code} onView={item => { void openForm('view', item.id); }} onAddChild={canManage ? item => { void openForm('create', null, item.id); } : undefined} canAddChild={item => !item.allowPosting} canCreate={canManage} canEdit={false} canDelete={false} rootLabel={t('coaHierarchy.accounts.root')} emptyMessage={t('coaHierarchy.accounts.emptyTree')} />
      ...
      searchActions={canManage ? <AppIconButton icon="add-outline" label={t('coaHierarchy.accounts.actions.add')} color={theme.colors.onPrimary} onPress={() => void openForm('create')} size={22} style={({ pressed }) => ({ backgroundColor: theme.colors.primary, opacity: pressed ? 0.75 : 1 })} /> : null}
    ...
    {formOpen && (formMode === 'create' || details.data) ? <AccountForm key={`${formMode}-${selectedId ?? 'new'}-${initialParentId ?? 0}`} item={formMode === 'create' ? null : details.data ?? null} mode={formMode} proposal={proposal.data?.code} proposalLoading={proposal.isFetching} proposalError={proposal.error ? errorMessage(proposal.error, t('coaHierarchy.messages.proposalFailed')) : null} onRetryProposal={() => void proposal.refetch()} hierarchyLevels={levelLookup.data ?? []} accounts={accountLookup.data ?? []} currencies={currencyLookup.data ?? []} lookupsLoading={levelLookup.isFetching || accountLookup.isFetching || currencyLookup.isFetching} lookupsError={lookupError ? errorMessage(lookupError, t('coaHierarchy.messages.lookupFailed')) : null} onRetryLookups={() => { void levelLookup.refetch(); void accountLookup.refetch(); void currencyLookup.refetch(); }} loading={saveMutation.isPending} initialParentId={initialParentId} canManage={canManage} onClose={closeForm} onSave={save} onEdit={details.data && !details.data.isDeleted ? () => { void beginEdit(); } : undefined} onLifecycle={details.data ? () => { void beginLifecycle(); } : undefined} /> : null}
