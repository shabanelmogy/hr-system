...
interface Props { item: AccountHierarchyLevel | null; mode: 'create' | 'edit' | 'view'; levels: readonly AccountHierarchyLevel[]; levelsLoading?: boolean; levelsError?: boolean; loading: boolean; canManage: boolean; onClose: () => void; onSave: (request: AccountHierarchyLevelRequest) => Promise<void>; onEdit?: () => void; onLifecycle?: () => void }

export function HierarchyLevelForm({ item, mode, levels, levelsLoading = false, levelsError = false, loading, canManage, onClose, onSave, onEdit, onLifecycle }: Props) {
  ...
  const mockDisabled = readOnly || loading || levelsLoading || levelsError || !canManage;
  const generateMock = () => {
    const draft = createHierarchyLevelMockDraft({ levels, usedSampleIndexes: usedMockSamples.current });
    form.setValue('levelNumber', draft.levelNumber, { shouldDirty: true, shouldValidate: true });
    form.setValue('nameAr', draft.nameAr, { shouldDirty: true, shouldValidate: true });
    form.setValue('nameEn', draft.nameEn, { shouldDirty: true, shouldValidate: true });
    form.setValue('canPost', draft.canPost, { shouldDirty: true, shouldValidate: true });
  };
  ...
  return <AppForm visible presentation="fullScreen" title={t(`coaHierarchy.levels.form.${mode}Title`)} subtitle={t('coaHierarchy.levels.form.subtitle')} icon={mode === 'create' ? 'add-circle-outline' : readOnly ? 'eye-outline' : 'create-outline'} errors={toFormErrorMap(form.formState.errors)} isDirty={form.formState.isDirty} submitting={loading || form.formState.isSubmitting} onCancel={onClose} onClearFieldError={name => form.clearErrors(name as keyof Values)} onSubmit={readOnly ? undefined : submit} submitLabel={t(mode === 'edit' ? 'common.update' : 'common.create')} serverError={serverError} mockDataAction={!readOnly ? { onGenerate: generateMock, disabled: mockDisabled } : undefined} contentContainerStyle={styles.content}>
