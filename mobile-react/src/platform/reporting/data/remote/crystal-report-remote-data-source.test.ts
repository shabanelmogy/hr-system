...
import { crystalReportRemoteDataSource } from './crystal-report-remote-data-source';
...
describe('Crystal report remote data source', () => {
  ...
  it('lists the permitted published catalog for the exact feature entity key', async () => {
    ...
    await expect(crystalReportRemoteDataSource.listPublished('countries')).resolves.toEqual([publishedReport]);
  ...
  it('fails closed when the published catalog does not match the runtime schema', async () => {
    ...
    await expect(crystalReportRemoteDataSource.listPublished('countries')).rejects.toThrow();
  });
