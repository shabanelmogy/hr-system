...
import type {
  ...
  PreparedFilePreview,
  StoredFile,
} from '@/src/platform/tools/file-manager/domain/models/file-manager';
import {
  getFileIcon,
  getFilePreviewKind,
} from '@/src/platform/tools/file-manager/presentation/utils/file-utils';
import { formatFileSize } from '@/src/platform/tools/utils/platform-tool-utils';
import {
  AppButton,
  AppCard,
  AppIcon,
...
export function FileViewerModal({ file, onClose }: FileViewerModalProps) {
    ...
    const load = async () => {
      ...
      try {
        if (previewKind === 'image' || previewKind === 'video' || previewKind === 'audio') {
          const source = await fileManager.getAuthenticatedFileSource(file);
        ...
        prepared = await fileManager.prepareFilePreview(file);
        if (!active) {
          prepared.dispose();
          return;
        }
  ...
  }, [file, fileManager, previewKind, retryKey, t]);

  const openWithDevice = async () => {
    setOpening(true);
    try {
      if (preview) {
        await fileManager.openPreparedFile(file, preview);
      } else {
