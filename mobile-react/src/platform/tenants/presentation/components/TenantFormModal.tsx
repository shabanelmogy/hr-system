...
} from '../../domain/policies/tenant-entitlements';
import type { TenantFormState } from '../models/tenant-form';
import { createTenantValidationSchema } from '../validation/tenant-validation';
import {
  AppDateTimeField,
  AppForm,
  type AppIconName,
        ...
        render={({ field }) => (
          <View style={styles.entitlements}>
            ...
            {tenantEntitlementModules.map((module) => {
              ...
              return (
                <View key={module.code} style={styles.entitlementCard}>
                  <AppSwitchField
                    description={module.code === 'hr' ? t('modules.hr') : module.code === 'acc' ? t('modules.accounting') : module.name}
                    disabled={submitting}
                    icon="apps-outline"
                    label={module.code.toUpperCase()}
                    name={`module-${module.code}`}
                    onValueChange={(checked) => field.onChange(
                      toggleModuleEntitlement(field.value, module, checked),
                    )}
                    value={Boolean(current)}
                  />
                  {current && module.submodules.map((submodule) => (
                    <AppSwitchField
                      key={submodule.code}
                      description={t(`modules.submodules.${module.code}.${submodule.code}`, { defaultValue: submodule.name })}
                      disabled={submitting}
                      icon="chevron-forward-outline"
                      label={submodule.code}
                      name={`submodule-${module.code}-${submodule.code}`}
                      onValueChange={(checked) => field.onChange(
                        toggleSubmoduleEntitlement(
                          field.value,
                          module.code,
                      ...
                      value={current.submoduleCodes.some(
...
const styles = StyleSheet.create({
  ...
  entitlements: { gap: 8, marginTop: 8 },
  entitlementCard: { gap: 4 },
});
