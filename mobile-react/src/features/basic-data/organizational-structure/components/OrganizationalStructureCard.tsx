...
export function OrganizationalStructureCard({ item, canEdit, canDelete, canApprove, active, onEdit, onArchive, onRestore, onView, onApprove, onReject }: Props) {
  const { t } = useTranslation(); const { theme } = useAppTheme();
  const parent = item.positionCode ?? item.divisionNameEn ?? item.departmentNameEn ?? item.branchNameEn ?? t('organizationalStructure.currentCompany');
  const descriptionStatus = getJobDescriptionStatusKey(item);
  return <AppDataCard active={active} padding="sm" style={styles.card}>
    ...
    <View style={styles.actions}><AppIconButton icon="eye-outline" label={t('organizationalStructure.view')} onPress={() => onView(item)} />{canEdit && !item.isDeleted ? <AppIconButton icon="create-outline" label={t('organizationalStructure.edit')} onPress={() => onEdit(item)} /> : null}{canDelete ? <AppIconButton icon={item.isDeleted ? 'refresh-outline' : 'archive-outline'} label={t(item.isDeleted ? 'organizationalStructure.restore' : 'organizationalStructure.archive')} onPress={() => item.isDeleted ? onRestore(item) : onArchive(item)} /> : null}{canApprove && canDecideJobDescription(item) ? <><AppIconButton icon="checkmark-circle-outline" label={t('organizationalStructure.decision.approve')} onPress={() => onApprove(item)} /><AppIconButton icon="close-circle-outline" label={t('organizationalStructure.decision.reject')} onPress={() => onReject(item)} /></> : null}</View>
  </AppDataCard>;
}
const styles = StyleSheet.create({ card: { gap: 7 }, header: { flexDirection: 'row', alignItems: 'center', gap: 7 }, icon: { width: 36, height: 36, alignItems: 'center', justifyContent: 'center' }, name: { flex: 1, minWidth: 0 }, details: { gap: 3 }, actions: { flexDirection: 'row', justifyContent: 'flex-end' } });
