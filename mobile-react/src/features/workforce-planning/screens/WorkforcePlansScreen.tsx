...
import { useFiscalYearLookup } from '@/src/features/finance';
...
import { useApproveWorkforcePlan, useArchiveWorkforcePlan, useBeginWorkforcePlanReview, useCreateWorkforcePlan, useCreateWorkforcePlanRevision, useRejectWorkforcePlan, useRestoreWorkforcePlan, useSubmitWorkforcePlan, useUpdateWorkforcePlan, useWorkforcePlan, useWorkforcePlanRevisions, useWorkforcePlans } from '../queries/use-workforce-plans';
import type { WorkforcePlan, WorkforcePlanPageQuery, WorkforcePlanRequest } from '../types/workforce-plan';
...
export function WorkforcePlansScreen() {
  ...
  const save = useCallback(async (request: WorkforcePlanRequest) => { try { if (formMode === 'create') await create.mutateAsync(request); else if (selected && details.data) await update.mutateAsync({ id: selected.id, request: { titleEn: request.titleEn, titleAr: request.titleAr, description: request.description, lines: request.lines, rowVersion: details.data.rowVersion } }); showToast.success(t(formMode === 'create' ? 'workforcePlanning.messages.created' : 'workforcePlanning.messages.updated')); closeForm(); } catch (error) { showToast.error(error, t('workforcePlanning.messages.saveFailed')); } }, [closeForm, create, details.data, formMode, selected, t, update]);
  ...
  if (query.error) return <AppScreen edges={['left', 'right', 'bottom']}><AppStateView state="error" message={errorMessage(query.error, t('workforcePlanning.messages.fetchError'))} onRetry={() => void query.refetch()} /></AppScreen>;
  return <AppScreen edges={['left', 'right', 'bottom']} contentContainerStyle={styles.screen} refreshControl={<RefreshControl refreshing={query.isRefetching} onRefresh={() => void query.refetch()} tintColor={theme.colors.primary} colors={[theme.colors.primary]} />}>
    ...
    {formOpen ? <WorkforcePlanForm key={`${formMode}-${selected?.id ?? 'new'}-${details.data?.rowVersion ?? 'pending'}`} item={details.data ?? null} revisions={revisions.data ?? []} mode={formMode} loading={create.isPending || update.isPending} detailLoading={formMode !== 'create' && details.isFetching} detailError={details.error ? errorMessage(details.error, t('workforcePlanning.messages.fetchError')) : null} onRetryDetail={() => void details.refetch()} onClose={closeForm} onSave={save} /> : null}
