...
import { useFiscalYear, useFiscalYearLookup } from '@/src/features/finance';
import { useOrganizationalLookup } from '@/src/features/basic-data';
...
import type { WorkforcePlanDetail, WorkforcePlanLine, WorkforcePlanRequest } from '../types/workforce-plan';

type Values = z.infer<ReturnType<typeof createWorkforcePlanSchema>>;
interface Props { item: WorkforcePlanDetail | null; revisions: WorkforcePlanDetail[]; mode: 'create' | 'edit' | 'view'; loading: boolean; detailLoading?: boolean; detailError?: string | null; onRetryDetail?: () => void; onClose: () => void; onSave: (request: WorkforcePlanRequest) => Promise<void>; }
...
const valuesFrom = (item: WorkforcePlanDetail | null): Values => item ? ({
...
export function WorkforcePlanForm({ item, revisions, mode, loading, detailLoading = false, detailError, onRetryDetail, onClose, onSave }: Props) {
  ...
  const form = useZodForm<Values>(schema, { defaultValues: valuesFrom(item) });
  ...
  useEffect(() => { form.reset(valuesFrom(item)); }, [form, item, mode]);
